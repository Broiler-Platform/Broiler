...
internal partial class CssBox
{
    ...
    /// rotation — declined below). The columns are the whole axis the layout will
    /// have, not just <c>grid-template-columns</c>: implicit columns generated by
    /// auto-placement or by a <c>grid-column</c> reaching past the template are
    /// sized from <c>grid-auto-columns</c>, exactly as the definite-width pass
    /// does, and a grid with no template at all is sized entirely that way.
    /// Bounded to tracks that are definite fixed lengths (a plain length or a
    /// <c>minmax()</c> with fixed sides — <c>repeat(&lt;int&gt;,…)</c> already
    /// expanded by <see cref="ParseTrackList"/>): declines for <c>fr</c>/
    /// <c>auto</c>/content/percentage/<c>auto-fill</c> tracks, whose size needs the
    /// real track pass (and the items). Since <c>grid-auto-columns</c> defaults to
    /// <c>auto</c>, that guard keeps today's content-based fallback for every grid
    /// that does not declare a fixed one. Lets a shrink-to-fit grid (float,
    /// <c>inline-grid</c>, <c>fit-content</c>/<c>min-content</c>/<c>max-content</c>)
    /// size to its tracks instead of collapsing to its (often empty) inline content
    ...
    private bool TryComputeGridIntrinsicContentWidth(bool useMax, out double contentWidth)
    {
        ...
        // A `none`/empty template is zero explicit tracks, not a reason to decline:
        // the axis is implicit-only and every column comes from grid-auto-columns.
        // Any other null — subgrid, masonry, an auto-repeat, an unsupported token —
        // still declines, since this path cannot resolve those tracks.
        if (specs == null)
        {
            if (!IsNoneOrEmptyTemplate(GridTemplateColumns))
                return false;

            specs = [];
        }

        if (specs.Count > MaxGridLine)
            return false;

        if (!TryCountGridIntrinsicColumns(specs.Count, em, out int colCount, out int explicitColStart))
            return false;
        ...
        {
            // The explicit tracks occupy [explicitColStart, explicitColStart+count);
            // everything outside that — leading implicit tracks from a negative line,
            // trailing ones from placement — is sized by grid-auto-columns.
            int explicitIndex = i - explicitColStart;
            GridTrackSpec spec = explicitIndex >= 0 && explicitIndex < specs.Count
                ? specs[explicitIndex]
                : implicitCol;
    ...
    /// <summary>
    /// The number of columns the grid's layout will actually have — the explicit
    /// template plus every implicit column placement generates — together with the
    /// index the explicit grid starts at (non-zero only when a negative
    /// <c>grid-column</c> references a leading implicit track, CSS Grid §8.3).
    /// Runs the same auto-placement the definite-width pass runs, over the same
    /// item set, so <see cref="TryComputeGridIntrinsicContentWidth"/> and the layout
    /// that follows it agree on the axis. Returns <c>false</c> for the shapes that
    /// pass declines too — an implausibly large placement, a leading implicit track
    /// alongside an auto-placed item — and for a grid with neither explicit tracks
    /// nor anything placed in them, which has no axis to sum.
    /// </summary>
    private bool TryCountGridIntrinsicColumns(int explicitCols, double em,
        out int colCount, out int explicitColStart)
    {
        ...
        // Placement consults the explicit *row* count in two places: resolving a
        // negative grid-row line, and — under column flow — deciding when
        // auto-placement wraps into the next column. ParseTrackList reports it for
        // every template whose tracks this path could size anyway; `none`/empty is
        // zero tracks and a bare `auto` is one. Anything else leaves it unknown, and
        // a grid whose column count would depend on it declines below.
        int explicitRows;
        bool rowsKnown;

        if (ParseTrackList(GridTemplateRows, em) is { } rowSpecs)
            (explicitRows, rowsKnown) = (rowSpecs.Count, true);
        else if (IsNoneOrEmptyTemplate(GridTemplateRows))
            (explicitRows, rowsKnown) = (0, true);
        else if (IsBareAutoTemplate(GridTemplateRows))
            (explicitRows, rowsKnown) = (1, true);
        else
            (explicitRows, rowsKnown) = (0, false);

        if (explicitRows > MaxGridLine)
            return false;
        ...
        foreach (var child in Boxes)
        {
            ...
            // Same exclusions as the definite-width pass: collapsible white space
            // between block-level items is not a grid item (CSS Grid §4), and an
            // out-of-flow child takes no part in placement or track sizing.
            if (IsCollapsibleWhitespaceItem(child))
                continue;
            ...
            // A grid-row this path could not resolve against the real explicit row
            // count would place the item on the wrong row, and phase 2 of placement
            // can push a row-flow item past the template's last column to find a free
            // slot — so the column count is not independent of it.
            if (!rowsKnown && rowStart.HasValue)
                return false;
        ...
        // Nothing placed: the explicit template is the whole axis. With no template
        // either there is no axis to sum, and the caller keeps its content-based
        // measurement.
        if (placements.Count == 0)
            return explicitCols > 0;

        // CSS Grid §8.3: a definite line resolving before the explicit grid
        // references a leading implicit track. Normalise so the leftmost/topmost
        // referenced line is index 0. Auto-placement into leading tracks is out of
        // scope for the definite-width pass, so decline here for the same shape.
        int minCol = 0, minRow = 0;
        ...
        // Column flow wraps into a new column once the rows run out, so an unknown
        // explicit row count makes the column count unknowable.
        if (!flowRow && !rowsKnown)
            return false;
