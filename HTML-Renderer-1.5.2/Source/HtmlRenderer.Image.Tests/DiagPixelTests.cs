...
public class DiagPixelTests : IDisposable
{
    ...
    public DiagPixelTests(ITestOutputHelper output)
    {
        _output = output;
    ...
    [Fact]
    public void ShowDiffPixels()
    {
        using var actual = RenderAtAnchorTop();
        using var baseline = SKBitmap.Decode(_referencePath);
        int tol = 5;

        // Show diff pixels for key rows
        int[] rows = { 69, 108, 204, 216, 228, 240, 252, 264 };
        foreach (int y in rows)
        {
            ...
            for (int x = 0; x < Math.Min(actual.Width, baseline.Width); x++)
            {
                ...
                if (c && !(Math.Abs(a.Red - r.Red) <= tol && Math.Abs(a.Green - r.Green) <= tol && Math.Abs(a.Blue - r.Blue) <= tol))
                {
            ...
            _output.WriteLine($"  Total diff pixels: {count}");
        }
    }
