...
| [Privacy test pages](privacy-test-pages.md) | The DuckDuckGo privacy test page corpus: how a page's own `results` global is read back with `Broiler.Cli --evaluate-page`, what "carried out" means, the tracked baseline and what counts as a regression against it, and the CI workflow |
