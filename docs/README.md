...
| [The `mediawiki.org` 403](mediawiki-user-agent-403.md) | Why every Broiler entry point answered `403 Forbidden` instantly on a Wikimedia page: `HttpClient` sends no `User-Agent` unless configured, and the policy refuses a request that carries none. Which loader fetches what, why a header on the document's client alone is not the fix, and what the capture diagnostics do and do not count |
