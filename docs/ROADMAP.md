...
[status ledger](../Broiler.VM/docs/roadmap.status.md). Broiler.VM is a new, planned
NativeAOT-compatible execution host for verified bytecode. It is a host for language profiles, not
a language: profiles are separate components that reference the core, and the core references
none of them. JavaScript and WebAssembly are the two intended first profiles, but neither is
planned by the core roadmap and no core milestone depends on either existing.

This is distinct from [Browser WebAssembly](#browser-webassembly): the latter
runs Broiler applications on the browser's `browser-wasm` platform, while a
Broiler.VM WebAssembly profile would load and execute WebAssembly guest modules.

It is also distinct from `Broiler.JS`, which remains a **legacy component** with its own roadmap,
ledger, and consumers. Broiler.VM does not depend on it, wrap it, or replace it on any schedule
stated here, and no core gate may cite its results. A future JavaScript profile is expected to
start from a snapshot copy of that component taken after its in-flight fix work lands, as a fork
used as a base with no dependency edge in either direction.

**Owners:** Broiler.VM owns the profile catalog, immutable verified-artifact boundary, common
execution lifecycle, trusted resource-limit precedence, profile-neutral operation-result
envelopes, bounded binary-reading primitives, diagnostics, profile isolation, and the numbered
core contract version that carries them. Each profile owns its format, verifier, values/frames,
language result/fault payloads, semantics, host boundary, oracle, conformance, and optimization.

**Current evidence:** the status ledger records VM-0 through VM-6 as not started. No Broiler.VM
project, contract ADR, catalog, fixture profile, or profile of any kind exists.

**Next actions:**

1. Freeze the profile/core boundary, lifecycle/result model, ID policy, static catalog, dependency
   graph, host-capability seam, monotonic resource policy, immutable verified-artifact ownership,
   and outer-envelope/profile-payload split in reviewed ADRs. Assign core contract version 1 with
   a published amendment procedure, and decide explicitly whether it admits guest-initiated loads,
   asynchronous instantiation, external suspension, and budgets shared across runtimes. Record the
   forbidden edge to every legacy component as an architecture test.
2. Build the reflection-free core, bounded binary primitives, and bounded loading and verification
   contracts, proven by a fixture profile. Execute only an owned immutable verified handle and
   prove that caller-buffer mutation cannot change behavior.
3. Prove the public source-level profile contract with an application-local consumer profile,
   added without modifying the core or using runtime discovery, and report the exact closure of
   each named composition under trimming and NativeAOT.
4. Harden lifecycle, concurrency, diagnostics, and the host boundary, then baseline what the core
   itself costs a profile. Package and freeze the API, the profile contract, and the contract
   version.
5. Open profile roadmaps as separate components once the core contract is accepted. Share
   mechanism between profiles and never semantics: no shared syntax tree, value model, frame
   layout, or opcode set, and a new shared component only through the roadmap's extraction gate.

**Exit gate:** the core publishes and runs every named composition on each claimed NativeAOT RID
with no unreviewed trim/AOT warnings; malformed and resource-hostile inputs fail before unsafe
execution; repeated lifecycle tests reach a resource plateau; catalogs and artifact versions
cannot drift silently; an application-local profile joins through the public direct-registration
contract without a core change; and the support table names the core contract version and states
that no language profile ships with the core. Profile conformance is a profile gate, recorded in
that profile's own roadmap and ledger.
...
2. Freeze the managed runtime and the linker/AOT mode, and record the
   ...
   general JavaScript runtime or in-process source compiler. Android Browser must select an explicit
   `execution-only`, `narrow-runtime-compiler`, or `general-runtime-compiler` composition under
   the [JavaScript modernization roadmap](../Broiler.JS/docs/roadmap/Modernization.md), instead
   of treating “Portable” as a complete engine. A future
   [Broiler.VM](../Broiler.VM/docs/roadmap.md) JavaScript profile is the eventual target, but it
   is a separate component that is not planned yet, and Android cannot wait on it. Writer may
   adopt a stricter mode than Browser.
