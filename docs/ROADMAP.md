...
## WebRTC browser capability

The detailed sequencing authority is the
[WebRTC implementation roadmap](webrtc-roadmap.md). It covers the native-backend
decision, durable document sessions, Web APIs, capture, live rendering/audio,
transport and media, security/privacy, real WPT, interoperability, platforms,
packaging, and release evidence. This root item records only the cross-component
outcome.

**Owners:** Browser and HtmlBridge integration own the product surface and runtime;
the proposed Broiler.WebRtc component owns the neutral/native engine seam;
Broiler.Input owns raw device discovery/capture; Graphics/HTML and media/output
owners own live presentation; platform, test, security, and release owners close
their profile gates.

**Current evidence:** WebRTC, media devices, `getUserMedia`, peer connections, data
channels, ICE/STUN/TURN/DTLS/SRTP/SCTP/RTP, and browser WebSocket are absent. Windows
has reusable camera/microphone providers and deterministic fakes, but no Browser
composition, permission broker, live HTML media route, or physical audio output.
The current bounded load window can dispose a settled JavaScript session before a
late RTC callback, and the visual WPT runner is not behavioral WebRTC evidence.

**Next actions:**

1. Complete RTC-0: freeze standards/API/test revisions, threat model and support
   profiles; compare maintained native backends behind a common C ABI; record the
   selected backend, licensing/security/update boundary, feasibility evidence, and
   measured estimate in reviewed ADRs.
2. Complete RTC-1 before exposing any global: retain document realms until
   navigation/close, add ordered external task sources and host wakeup, marshal all
   callbacks to the JavaScript owner thread, and prove deterministic teardown.
3. Establish the neutral/native/testing component, then deliver a data-channel-first
   preview, secure Windows fake/physical capture, live `srcObject`, interoperable
   audio/video, full API semantics, and network/security hardening through the
   roadmap's per-phase gates.
4. Add a real pinned `wptserve`/HTTPS/testharness lane, direct/TURN network lab,
   Chromium/Firefox interop, native fuzz/sanitizers, hardware, performance/soak,
   package, SBOM, and rollback evidence. Keep Linux and Android absent until their
   independent platform gates pass.
5. Track browser WebSocket as a sibling standards feature and keep production
   signaling and TURN services outside the browser implementation unless separately
   approved.

**Exit gate:** every applicable mandatory test in the exact claimed platform's
frozen behavioral WPT manifest passes without failure, timeout, crash, skip, or
expected-failure waiver; direct and TURN-relayed data/audio/video interop with the
reference-browser matrix, secure permission and IP/device privacy behavior, bounded
performance/resources, deterministic navigation/shutdown cleanup, reviewed native
provenance/security/update paths, correct packages and support detection, and a
signed release evidence package also pass. Completion on one platform does not imply
another.

## Broiler.VM bytecode execution

The component sequencing authority is the
[Broiler.VM roadmap](../Broiler.VM/docs/roadmap.md), and current evidence is recorded in its
[status ledger](../Broiler.VM/docs/roadmap.status.md). Broiler.VM is a planned
NativeAOT-compatible execution host for validated bytecode. JavaScript and WebAssembly are its
required initial Broiler-provided profiles; future Broiler or application-local built-ins join
through the same public source-level, closed-world registration contract. That contract is not a
binary plug-in ABI.

This is distinct from [Browser WebAssembly](#browser-webassembly): the latter
runs Broiler applications on the browser's `browser-wasm` platform, while the
Broiler.VM WebAssembly profile loads and executes WebAssembly guest modules.

**Owners:** Broiler.VM owns the profile catalog, immutable verified-artifact boundary, common
execution lifecycle, trusted resource-limit precedence, profile-neutral operation-result envelopes,
diagnostics, and profile isolation. Each built-in owns its format, verifier, values/frames,
language result/fault payloads, semantics, host boundary, conformance, and optimization. Broiler.JS
owns the shared JavaScript semantics, canonical compiler-to-format lowering, JavaScript runtime
adapter, IL comparison, and optional IL tiering.

**Current evidence:** the status ledger records VM-0 through VM-8 as not started: no Broiler.VM
project or WebAssembly guest interpreter exists. `Broiler.JavaScript.Portable` is a 20-opcode,
`double`-only seed, and its Native AOT sample executes checked-in generated bytecode without the
parser or compiler. That is useful execution-only evidence, not a general JavaScript profile,
WebAssembly support, a reusable profile host, or an in-process source-compiler closure.

**Next actions:**

1. Freeze the profile/core boundary, lifecycle/result model, stable profile and feature/version
   IDs, static catalog, dependency graph, host-capability seam, monotonic host/profile/artifact
   resource policy, immutable verified-artifact ownership, and outer-envelope/profile-payload split
   in reviewed ADRs. Assign a numbered core contract version with a published amendment procedure,
   and decide explicitly whether that version admits guest-initiated loads for `eval` and dynamic
   `import()`, asynchronous instantiation for top-level await, external suspension for debugging,
   and budgets shared across several runtimes. VM-0/VM-1 establish the neutral contract before
   terminal Broiler.JS MOD-M9.
2. Build the reflection-free core and bounded common loading/verifier contracts with a fixture
   profile. Execute only an owned immutable verified handle and prove that caller-buffer mutation
   cannot change behavior. Prove the public source-level application-profile contract without
   modifying the execution loop or using runtime discovery.
3. Deliver the JavaScript and WebAssembly profiles as independent vertical
   slices. Keep the JavaScript execution-only and optional runtime-compiler AOT closures separate
   and give compiler and executor one canonical format contract. VM-0 owns one pinned WebAssembly
   Core/proposal/import/WASI/conformance manifest; VM-4 consumes it, rejects unapproved features and
   imports deterministically, and keeps WAST tooling outside product closures. The initial profile
   does not claim WASI.
4. Publish and run JS-only, Wasm-only, and aggregate compositions on every
   claimed NativeAOT RID with trim/AOT warnings treated as errors. Add malformed
   input, fuzz, resource, lifecycle, concurrency, conformance, package, and
   pristine-consumer gates before optimization claims.
5. Treat JavaScript-to-WebAssembly interop, persisted artifacts, quickening,
   superinstructions, tier-up, deoptimization, and OSR as separately funded
   follow-ons. Same-process execution does not imply cross-profile interop.

**Exit gate:** each advertised built-in passes its pinned independent conformance manifest;
malformed and resource-hostile inputs fail before unsafe execution; each claimed static composition
publishes and runs under NativeAOT with no unreviewed trim/AOT warnings; repeated lifecycle tests
reach a resource plateau; profile catalogs and artifact versions cannot drift silently; and an
application-local fixture joins through the public direct-registration contract. JavaScript-only
and Wasm-only release records may close independently. The aggregate claim additionally requires
both profile gates and mixed-profile evidence; no result from one profile is counted for another.
...
2. Freeze the managed runtime and the linker/AOT mode, and record the
   ...
   general JavaScript runtime or in-process source compiler. Android Browser must use the
   statically registered JavaScript built-in from the
   [Broiler.VM roadmap](../Broiler.VM/docs/roadmap.md) and select its `execution-only`,
   `narrow-runtime-compiler`, or `general-runtime-compiler` composition under the
   [JavaScript modernization roadmap](../Broiler.JS/docs/roadmap/Modernization.md), instead
   of treating “Portable” as a complete engine. Writer may adopt a stricter mode than Browser.
