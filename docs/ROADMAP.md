...
[Broiler.VM roadmap](../Broiler.VM/docs/roadmap.md). Broiler.VM is a planned
NativeAOT-compatible execution host for validated bytecode. JavaScript and
WebAssembly are its required initial built-in language profiles; future built-ins
join through the same closed-world, build-time registration contract.
...
**Owners:** Broiler.VM owns the profile catalog, common execution lifecycle,
resource/diagnostic contracts, and profile isolation. Each built-in owns its
format, verifier, values/frames, semantics, host boundary, conformance, and
optimization. Broiler.JS owns the shared JavaScript semantics, source-to-bytecode
lowering, JavaScript runtime adapter, IL comparison, and optional IL tiering.

**Current evidence:** no Broiler.VM project or WebAssembly guest interpreter
exists. `Broiler.JavaScript.Portable` is a 20-opcode, `double`-only seed, and its
Native AOT sample executes checked-in generated bytecode without the parser or
compiler. That is useful execution-only evidence, not a general JavaScript
profile, WebAssembly support, a reusable profile host, or an in-process
source-compiler closure.

**Next actions:**

1. Freeze the profile/core boundary, stable profile and feature/version IDs,
   static catalog, dependency graph, host-capability seam, resource policy, and
   the raw-WebAssembly-versus-common-envelope decision in reviewed ADRs.
2. Build the reflection-free core and bounded loading/verifier contracts, with a
   fixture profile proving that another built-in can be added without modifying
   the execution loop or using runtime discovery.
3. Deliver the JavaScript and WebAssembly profiles as independent vertical
   slices. Keep the JavaScript execution-only and optional runtime-compiler AOT
   closures separate; pin the WebAssembly specification/proposal manifest and
   reject unapproved features deterministically.
...
**Exit gate:** both required built-ins pass their pinned independent conformance
manifests, malformed and resource-hostile inputs fail before unsafe execution,
each claimed static composition publishes and runs under NativeAOT with no
unreviewed trim/AOT warnings, repeated create/run/dispose reaches a resource
plateau, profile catalogs and artifact versions cannot drift silently, and a
third fixture built-in joins through direct compile-time registration. No result
from one language profile is counted as evidence for another.
