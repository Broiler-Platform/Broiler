...
The Android application roots moved out of this repository with their
applications: `Broiler.Android.Writer.slnx` is generated in the
[Broiler.Writer](https://github.com/Broiler-Platform/Broiler.Writer) submodule and
`Broiler.Android.Browser.slnx` in the
[Broiler.Browser](https://github.com/Broiler-Platform/Broiler.Browser) submodule,
each generated by that repository's own `eng/solutions.json`. The host-runnable
`Broiler.Android.Tests.slnx` is still generated here. Their forbidden-project
patterns exclude Windows, Linux, and WebAssembly heads from Android closures.
...
files are generated, that flag is declared by a `deploy` entry in the owning
repository's `eng/solutions.json` rather than edited into the `.slnx` — a
hand-edit there is silently reverted by the next generator run, which is how the
flag was nearly lost. That entry now lives in
[`Broiler.Writer/eng/solutions.json`](../../Broiler.Writer/eng/solutions.json),
not in this repository's manifest. `dotnet build` ignores it, so no CI job
depends on it; the packaging workflow here builds
`Broiler.Writer/src/Broiler.Writer.Android/Broiler.Writer.Android.csproj`
directly. `Broiler.Android.Browser.slnx` declares no deploy entry. That asymmetry
is inherited rather than intended, and settling it belongs to the Android owner.
...
```powershell
dotnet build Broiler.Writer/Broiler.Android.Writer.slnx -c Debug `
  -p:AndroidSdkDirectory="C:\Program Files (x86)\Android\android-sdk"
dotnet build Broiler.Browser/Broiler.Android.Browser.slnx -c Debug `
  -p:AndroidSdkDirectory="C:\Program Files (x86)\Android\android-sdk"
dotnet run --project Broiler.Input/src/tests/Broiler.Input.Android.Tests
dotnet run --project Broiler.Graphics/src/tests/Broiler.Graphics.Android.Tests
```
