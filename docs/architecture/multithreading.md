...
> **Ownership boundary.** This document retains aggregate integration history, host-side
> implementation state, and the measurements that produced it. JavaScript-local compiler,
> cache, context, optimizer-state, and Worker-agent acceptance is now owned by
> [`Broiler.JS/docs/roadmap/Concurrency.md`](../../Broiler.JS/docs/roadmap/Concurrency.md) and
> its [status record](../../Broiler.JS/docs/roadmap/Concurrency.status.md). “Done”, “built”,
> or “gate passed” below closes this document's historical item only; it does not close
> modernization MOD-M5, MOD-M6, or MOD-M7.

## Status

**Phase 4 is complete, and with it every phase in this aggregate roadmap.** It ends the way it
...
| #18 — Web Workers | **Built (aggregate first slice); aggregate gate passed; MOD-M7A open** | Contexts passed the scoped isolation/scaling probe — **2.66×/3.22× at 4 threads** ([§10](#10-item-18s-aggregate-gate-passes-on-both-halves-mod-m6mod-m7-acceptance-remains-open)). `MessageChannel` and structured clone were **already built**, with cross-realm cloning verified ([§11](#11-messagechannel-was-already-built-the-unverified-word-was-cross-context)). **`Worker` now exists**: own thread, own `JSContext`, messages cloned twice — sender-side into an unreachable graph, receiver-side into its realm — and replies queued onto the page's loop, so item #15 holds. Plus worker timers on **real** deadlines, `importScripts` resolved against the worker's own directory, and copy-then-detach transferables. The newer MOD-M7A gate still requires MOD-M6 optimizer-state safety, an agent/lifecycle contract, one host-wide cap, failed-transfer atomicity, resource plateau/tail evidence, applicable WPT scope, and explicit rejection of cross-agent SharedArrayBuffer/Atomics ([Concurrency status](../../Broiler.JS/docs/roadmap/Concurrency.status.md)). |
...
| #16 — parallel script compile | **Done for aggregate host slice; MOD-M5 open** | `ScriptCompileAhead` (budget `BROILER_SCRIPT_COMPILE_THREADS`) compiles a document's classic script sources into the context's own code cache while the host parses and while the ordered eval loop runs the scripts ahead of them; the loop is untouched and finds each compile done. **Compile stage 1.41×/1.62×/1.52× at 2/4/8 threads, whole capture 1.44× on a compile-heavy document and 1.22× on a modestly scripted one**; 45 tests, 8 documents × budgets 1/2/3/4/8 compared byte for byte, plus an assertion on the cache's own counters that the worker's key is the key `Eval` asks for. **No submodule change and no patch** — see [§8](#8-item-16s-blocker-did-not-exist-the-store-is-the-contexts-own-cache). MOD-M5 additionally requires the shared-state/artifact census, representative critical-path decision, single-flight/error policy, one host-wide budget, and process-resource/tail gates in [the JS-local plan](../../Broiler.JS/docs/roadmap/Concurrency.md). Published: [`tests/render-stages/results/script-compile-ahead.md`](../../tests/render-stages/results/script-compile-ahead.md) |
...
| **#18's refused surface** | [§12](#12-the-aggregate-worker-first-slice-is-built-mod-m7a-acceptance-remains-open) | Module workers, `SharedWorker`, nested workers, worker `requestAnimationFrame`, `MessagePort` transfer, network-fetched worker scripts, and cross-agent `SharedArrayBuffer`/Atomics. Also: the engine's `structuredClone` implements transfer as copy-then-detach, which is spec-observable but is not the performance reason transferables exist |
...
**Historical delivery provenance, not open work.** Aggregate patches `0132`, `0133`, and
`0134` were removed with the obsolete patch ledger by `ef50ea25` after their subjects landed
in the pinned `Broiler.HTML`: render-tree/cascade invalidation in `0a29f7b` and `54fc320`, the
layout-pass counter in `11cf08e`, and direct pixel comparison in `beab9f8`. The associated
measurements remain in this document and under `tests/render-stages/results/`; deleted patch
paths are evidence labels only, not current reproduction or application instructions.
...
2. **The cache-ownership branch of the cross-document question does not arise for this
   host slice.** It was a consequence of the *other* option §6 floated (making the bridge
   take the process-shared cache); a per-context cache handed to a per-document worker has
   exactly the lifetime it had before. The wider MOD-M5 shared-state/artifact census and
   controlled acceptance gates still apply.
...
### 10. Item #18's aggregate gate passes on both halves; MOD-M6/MOD-M7 acceptance remains open
...
**The scoped premise holds.** The named default paths stayed isolated under this real-concurrency
probe and ran in parallel. This establishes that item #18 is not trivially gated on one global
engine lock; it is not a census of optimizer state, every host entry, lifetime reclamation, or
the MOD-M6/MOD-M7 acceptance matrix. Those remain with the
[JS-local concurrency plan](../../Broiler.JS/docs/roadmap/Concurrency.md).
...
**What this was not at the time of the probe**: the feature. The workload exercised the shared
state the roadmap named, not the bridge, timers, promises or modules. Sections 11–12 record the
later `MessageChannel` verification and Worker first slice; the old “unwritten” state and 30–40 day
estimate are superseded, while the probe remains only scoped MOD-M6 evidence. Published:
...
**What was left at that point was bounded and named.** The clone is made on the *sender's* side — `CloneForMessaging`
...
### 12. The aggregate `Worker` first slice is built; MOD-M7A acceptance remains open
...
**Verified** by `WorkerBindingTests` — seventeen cases for the object itself, twenty-three once timers,
`importScripts` and transferables landed, all driving the real bridge — round trip; cloning proved
...
**Transferables are in, with the word qualified.** `postMessage(v, [buf])` and the
`{ transfer: [buf] }` spelling both detach the listed `ArrayBuffer`s on the sending side, in both
directions. But the engine's `structuredClone` implements transfer as **copy then detach, not
zero-copy** — the observable semantics are the spec's (sender detached, receiver has the contents),
what is missing is the performance reason transferables exist. That is the engine's property, not
this binding's, and it is written down because "transferred" reads like a promise of zero copies. A
`MessagePort` in the list is refused rather than silently copied: a copied port would look like it
worked and deliver messages to nobody. Six cases, two of which exist to stop the others being
vacuous — a control send *without* a transfer list proving the buffer stays usable, and an
already-detached buffer proving re-transfer is a `DataCloneError` rather than a silent no-op. One
change fell out of it: `RegisterDOMException` was private, so workers had no real `DOMException` and
`ThrowDOMException` threw a bare string; making it `internal` and installing it in every worker
global is what lets worker code branch on `e.name`.

**Out of this slice, refused rather than half-built**: module workers, `SharedWorker`, nested
workers, `requestAnimationFrame` in a worker, `MessagePort` transfer, network-fetched worker
scripts, and cross-agent `SharedArrayBuffer`/Atomics. The newer MOD-M7A gate additionally keeps
the slice experimental until the agent lifecycle, failed-transfer atomicity, host-wide cap,
resource plateau/tails, applicable WPT scope, and explicit shared-memory rejection tests are
recorded in [the owning status](../../Broiler.JS/docs/roadmap/Concurrency.status.md). Published:
...
| 9 | Graphics | [`FontsHandler.cs`](../../Broiler.Graphics/src/Broiler.Graphics/Text/FontsHandler.cs), [`TrueTypeFont.cs`](../../Broiler.Graphics/src/Broiler.Graphics/Text/TrueTypeFont.cs), [`FallbackSystemFont.cs`](../../Broiler.Graphics/src/Broiler.Graphics/Rendering/FallbackSystemFont.cs), [`BImageRenderer.cs`](../../Broiler.Graphics/src/Broiler.Graphics/Rendering/BImageRenderer.cs) — **DONE** | Plain `Dictionary`, no synchronization; and two lazy-init latches published before their values | Not a speedup itself — a **hard prerequisite** for #10, #12, #13, and per Phase 1 §2 for *every* CPU-parallel render-path item | Nothing now | **DONE.** Nested map flattened to one concurrent dictionary; image table concurrent and handle allocation interlocked; replay transform state moved per-call; `TrueTypeFont`'s five lazy tables re-published through `Lazy` in `ExecutionAndPublication`. Both P0-c residuals closed with it. Enables ~4 items | Low | Done | 2 |
| 10 | Graphics | [`TrueTypeFont.GetGlyphContours`](../../Broiler.Graphics/src/Broiler.Graphics/Text/TrueTypeFont.cs) — **DONE**; [`ComplexTextShaper.Shape:72`](../../Broiler.Graphics/src/Broiler.Graphics/Text/ComplexTextShaper.cs) — not built | Outlines were re-extracted per glyph *occurrence*; shaping is called per run during layout | Concurrent cache by glyph index, published through `GetOrAdd` | Nothing | **The item was right that the cache is the whole win, and wrong about which cache.** Glyph outlines: raster stage **1.34×** (`text`), **1.54×** (`boxes`), 1.00× on the text-free `paint` control. The shaped-run cache is deliberately unbuilt — `RequiresShaping` is false for the whole Latin corpus, so it would measure nothing ([§5](#5-the-phases-largest-win-so-far-is-a-cache-and-not-the-one-item-10-names)) | Low | Done (outlines) | 2 |
...
| 12 | CSS | [`CssStyleEngine.GetCascadedStyle`](../../Broiler.CSS/Broiler.CSS.Dom/CssStyleEngine.cs) resolved ahead of the box walk by [`CssStyleRecalc`](../../Broiler.Layout/Broiler.Layout/Engine/CssStyleRecalc.cs) — **DONE** | Was sequential per element, with the three memo caches under one global `_sync` | Warm pass over every element on `BROILER_STYLE_THREADS`, then the unchanged ordered box walk reads the memo; caches sharded to `ConcurrentDictionary`, generation guard kept | Nothing now. **Neither blocker this cell named was the operative one.** The lock was not the bottleneck — the per-element cascade was 210 µs on a five-rule page ([§3](#3-the-_sync-lock-was-not-the-bottleneck-the-item-names-and-the-cascades-own-cost-is)) — and the parallel unit is not the box walk, which cannot be split at all ([§4](#4-the-parallel-unit-is-not-the-box-walk--this-is-a-prefetchconsume-split-the-fourth-in-this-document)) | **Measured: 1.16–2.01× on the cascade stage at 4 threads, 1.08–1.96× end to end**, pixel-identical at 1/2/4. The serial residue is now measured per page (16–55%), so what is left is stated rather than guessed ([§5](#5-the-serial-residue-is-measured-now-and-it-differs-three-fold-across-pages)) | Medium | Done | 3 |
...
| 15 | JS | `JSPromise`, `JSAsyncFunction`, and `JSGenerator` now route continuation entry through `JSContext.PostJob` — **DONE** | Was: continuations could fall onto pool threads and race the context owner | Per-context exclusion/job dispatch with owner-thread/event-loop delivery where required | JavaScript-local MOD-M6 still must enumerate every host/async entry and prove one owner policy; completion of these three sites is not that census | Correctness result; no speed claim | Low | Done | 0 |
...
| 18 | JS | [`WorkerBinding`](../../src/Broiler.HtmlBridge.Dom/Features/WorkerBinding.cs), [`JSWorker`](../../src/Broiler.HtmlBridge.Dom/Features/JSWorker.cs), and existing `MessageChannel` — **aggregate first slice built** | One Worker thread/context/realm, two-stage cross-realm clone, page-loop reply queue, errors/termination, timers, `importScripts`, and copy-then-detach transfer | One ECMAScript agent per owner thread; structured-clone message passing; no cross-agent shared memory in the initial profile | **MOD-M7A open:** MOD-M6 optimizer-state ownership, explicit agent lifecycle/FIFO/error contract, failed-transfer atomicity, one host-wide cap, WPT scope, resource/tail/plateau evidence, and explicit cross-agent SharedArrayBuffer/Atomics rejection | Capability; scoped 4-thread context probe measured 2.66× per-context / 3.22× shared-cache, but this is not MOD-M7 acceptance | High | Implemented subset | 4 |
...
| 24 | Documents | [`DocxReader`](../../Broiler.Documents/src/Broiler.Documents.Docx/DocxReader.cs), `RtfReader`, `HtmlReader`, `MarkdownReader` | Sequential single-document parsers | Batch conversion only — covered by #20 | — | None intra-document | — | — | Not recommended |
...
3. **Parallel/background script compile** (item #16) — **DONE for the aggregate host
   slice; MOD-M5 acceptance remains open**. The
...
4. **Workers** (item #18) — **aggregate first slice built; MOD-M6/MOD-M7 acceptance open**.
   The scoped P0-c probe did not find one global lock or leakage in its named paths, but the
   JS-local gate still requires complete adaptive-state ownership, host-entry coverage,
   agent lifecycle, a host-wide cap, resource plateau, and explicit no-shared-memory tests.
