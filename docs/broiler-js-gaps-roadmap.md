...
| 5 | Essential browser JavaScript APIs | Mixed partial, absent, and stubbed surfaces; `performance.now()` and the document-surface audit line fixed | open · closed | A tested support matrix replaces accidental omissions |
| 6 | DOM, CSSOM, and SVG from JavaScript | Partial object and tree models; `Node` interface constants fixed, `compareDocumentPosition` retired as not reproducing | open · closed | Script-visible objects and algorithms meet their claimed standards |
