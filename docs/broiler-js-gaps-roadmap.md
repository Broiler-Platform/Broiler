...
| 6 | DOM, CSSOM, and SVG from JavaScript | The rest of the element members onto the interface prototypes (character data, EventTarget, and the element's and document's Node members have moved); canonical Shadow DOM; live SVG DOM and the `viewBox` remainder of `foreignObject` placement; two tags in the UA `display` table; three capability decisions (`document.all`, per-tag SVG interfaces, Font Loading) | open · closed | Script-visible objects and algorithms meet their claimed standards |
