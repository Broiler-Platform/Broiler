...
| 6 | DOM, CSSOM, and SVG from JavaScript | Members onto the interface prototypes; canonical Shadow DOM; live SVG DOM and the `viewBox` remainder of `foreignObject` placement; the UA stylesheet's `display` missing from `getComputedStyle`; three capability decisions (`document.all`, per-tag SVG interfaces, Font Loading) | open · closed | Script-visible objects and algorithms meet their claimed standards |
