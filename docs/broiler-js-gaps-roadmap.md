...
| 6 | DOM, CSSOM, and SVG from JavaScript | Partial object and tree models; `Node` interface constants, CharacterData and tree-mutation `DOMException`s and the linked-stylesheet CSSOM gap fixed; the form default/reset/radio family characterized and fixed; `compareDocumentPosition`, qualified attributes and computed `display` retired as not reproducing | open · closed | Script-visible objects and algorithms meet their claimed standards |
