...
- **Provenance:** the pinned submodule pointer is **`14fa4f10`**, checked 2026-08-05 against
  ...
  **Nine patches are pending again, and four of them are one item.**
  ...
  **attributed**, with the sum closing to within 0.5 ns: **92% of it is Annex B `caller`/`arguments`**
  ...
  And **[`0110`](../patches/0110-js-legacy-frame-census.patch)** — that frame **counted** rather
...
empty for the second time since it was written. **Nine patches are open again** (`0102`–`0110`: item 1-1's remaining
...
arithmetic half priced and refused, item 4-5 unblocked, attributed and counted, and two instrument defects closed), on the usual 403 terms.
...
correctness surface** — recursion, re-entrancy, generators suspending mid-call — and it is a design
question rather than a measurement one, which is the first time this item has been in that position.
