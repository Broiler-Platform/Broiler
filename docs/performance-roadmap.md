...
|---|---|---|---|---|
| **0** | 0-1…0-5 ✅, 0-9…0-11 ✅ → 0-6 workflow run ✅ (17/17 committed, refreshed 2026-08-05) → **0-6's noise band — CI now defaults to `--repetitions 3` ✅, a banded CI result is what is awaited** → 0-7, 0-8 | — | Everything. 12 → **17 scores** ✅, known noise band, and the first evidence any phase A–F can close on | 17/17 ✅, no timeout at the 180 s floor ✅, `comparison.md` reporting the triad ✅, **band on record** (unblocked: the workflow defaults to 3 repetitions and is sized for them, 105 → 180 min; a local 12-suite band is recorded in §0) and **the BenchmarkDotNet + RID-matrix rows collected** — the latter two still open, and a container cannot produce either |
