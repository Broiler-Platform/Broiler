...
- **Provenance:** the pinned submodule pointer is **`14fa4f10`**, checked 2026-08-05 against
  ...
  **Six patches are pending again, and four of them are one item.**
  ...
  **[`0106`](../patches/0106-js-reentry-structural-partition.patch)** — that check's residual
  ...
  nothing unexplained.
  And **[`0107`](../patches/0107-js-generic-operator-ceiling.patch)** — item 4-2's arithmetic half
  priced and **refused by its own arithmetic** at **0.038%** of the corpus, the relational lead it
  points at closed with it at **0.006%**, and **the whole generic binary-operator surface bounded
  at 0.127%**.
...
empty for the second time since it was written. **Six patches are open again** (`0102`–`0107`: item 1-1's remaining
half in four of them — its capture-free population, its capture layout, a body compiled from a kept
enclosing scope, and that check's residual settled — plus the widened census corpus and item 4-2's
arithmetic half priced and refused), on the usual 403 terms.
...
- **A number computed over a subset stays wrong in the same direction every time you re-use it, and
  ...
  **1.89% to 0.65%** — the latter because *"from a promoted caller"* is 42.1% of the real corpus and
  not 64.0%. Neither re-take needed new code; the widened hosts had been shipping for one patch, and
  the numbers were simply never read again. **The seven suites are 10.4% of the corpus's calls and
  5.0% of its time.** *When an instrument's reach changes, re-derive everything that was ever
  computed from it, and re-derive it by reproducing the old reading first* — both re-takes matched
  the old figure over the old subset — 4-4's to within 0.0002% on a count of millions — which is
  what makes the new reading the same measurement rather than a different one.
...
- **The item's arithmetic half — measured, and refused by its own arithmetic** (`0107`). See
  §4-2c below; it is 0.038% of the corpus, and at the `+` rate it is net negative.
...
is **147 ms of a 391 572 ms run: 0.038%**. At the `add` rate it is **net negative**. For scale,
**4-2b landed at 0.83%** and this document already called that *"real, and below the noise floor"*.
This is 22× smaller than the thing that was already too small to see.
...
was.** 26.2 M arithmetic invocations at 15.42 ns and 24.0 M comparisons at 3.93 ns is **498 ms of
391 572 ms — 0.127% of the corpus's execution time if it were removed *entirely***. That sits beside
...
### 4-4 · Inlining of small JS callees — **premise measured, ceiling re-taken over the whole corpus at 0.65%; do not start it**
...
#### The ceiling was a seven-suite number too, and widened it is a third of it
...
| | the seven | **all fifteen** |
|---|--:|--:|
| All invocations | 6 194 744 | **59 711 685** |
| Native callee | 37.0% | **32.1%** |
| Calls with a JavaScript callee | 3 902 604 | **40 541 851** |
| …from a promoted caller | 2 496 730 — **64.0%** | 17 074 137 — **42.1%** |
| Driver | 19 650 ms | 391 572 ms |
| **4-4's ceiling** | **1.89%** | **0.65%** |
| Inlining every JS-callee call | 2.95% | **1.54%** |
| **The fixed call prologue (4-5's surface)** | **4.48%** | **2.17%** |

***The seven suites are 10.4% of the corpus's calls and 5.0% of its time.*** An item sized on them
was sized on a twentieth of the workload.
...
**Both readings are computed against a counters-on driver**, which is how the 1.89% was taken, so
the comparison is exact; both absolute percentages are understatements by whatever the counters cost,
identically.

**So 4-4's re-specification is not weakened, it is confirmed harder.** The section below recommends
4-5 over 4-4 on the strength of 4.47% against 1.89% — a 2.4× argument. Widened it is **2.17%
against 0.65%**, which is **3.3×**, and 4-4's own ceiling has fallen below the 0.83% of the 4-2b
this campaign already called below the noise floor. **An XL whose perfect execution is 0.65% should
not be started**, and that is now a measurement rather than a ranking.
...
**Widened, the pair is 5.9%, and the way it is computed needs saying.** The call half comes from
this host and is re-taken cleanly above: **2.17%**. The read half does *not* — 37 871 908 is 4-1's
...
same way over the same widened corpora — §4.2a's **307.9 M** reads against this host's 391 572 ms —
the read half is **3.74%**, and the pair is **5.91% rather than under 14%**. *Both halves fell, and
the conclusion the sentence was drawn for gets stronger rather than weaker.* (This host's own read
columns count the inline cache's hits and misses, 246 703 864 over fifteen suites for **3.00%**,
...
- **4-5 (new, M–L): make the call prologue cheaper.** The measurement says 142 ns of every call is
  ...
  **This applies to all 6 194 758 invocations rather than the 2 496 760 inlinable ones — 2.5× the
  ...
  risk. **Over the whole corpus the same comparison is 59 711 685 against 17 074 137 — 3.5× the
  calls — and 2.17% against 0.65%**, so widening moved both items down and moved 4-5 *further ahead*
  of 4-4. Halving the fixed cost is ~1.1% there, still more than 4-4's perfect execution.
