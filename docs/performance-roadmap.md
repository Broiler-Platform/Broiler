...
- **Provenance:** the pinned submodule pointer is **`14fa4f10`**, checked 2026-08-05 against
  ...
  **Four patches are pending again, and three of them are one item.**
  ...
  **[`0104`](../patches/0104-js-capture-layout-checker.patch)** — item 1-1's *named* obstacle
  ...
  And **[`0105`](../patches/0105-js-deferred-body-reentry.patch)** — the obstacle the item does
  *not* name: a nested body **compiled a second time from the enclosing scope kept alive**, after
  the enclosing compilation has finished, reproducing **84.1% of 5 723 retained functions**
  character-for-character up to compiler-generated numbering.
...
empty for the second time since it was written. **Four patches are open again** (`0102`–`0105`: item 1-1's remaining
half in three of them — its capture-free population, its capture layout, and a body compiled from a
kept enclosing scope — plus the widened census corpus), on the usual 403 terms.
...
| Corpus | re-entered | reproduced | share |
|---|--:|--:|--:|
| **mandreel** | 1 476 | 1 476 | **100.0%** |
| pdfjs | 910 | 909 | 99.9% |
| box2d | 982 | 977 | 99.5% |
| codeload-jquery | 534 | 518 | 97.0% |
| codeload-closure | 58 | 56 | 96.6% |
| typescript | 1 763 | 875 | 49.6% |
| **total** | **5 723** | **4 811** | **84.1%** |
...
**What is not established.** The residual **15.9%** is, on every instance examined, the *ordinal
assignment* diverging rather than the structure — `#site1329` against `#site1327`, `LABEL_#313`
against `LABEL_#1` — because the two sides encounter different numbers of distinct generated values
before that point. Each family added moved the total (45.5% → 48.9% → 83.2% → 84.1%) and **never
once revealed a structural mismatch**, which is suggestive and is not proof. *A printed-text diff of
two compilations can only ever answer "equal up to a renaming somebody thought of in advance."*
Settling the residual needs a **structural** comparison walking the two trees, which is the next
