...
`TypeFeedbackMetrics`, `SpecializingTierMetrics` and `PropertyMapDistributionMetrics` all ran the
same seven: Richards, DeltaBlue, RayTrace, Box2D, EarleyBoyer, Crypto, NavierStokes. That was a
defensible corpus for the question 4-1 asked. It stopped being one the moment its output was
quoted as ***"the corpus"*** — the phrase every phase-3 and phase-4 headline in this document
uses — over a denominator that was never stated.
