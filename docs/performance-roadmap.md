...
- **Provenance:** the pinned submodule pointer is **`07adeb44`**, checked 2026-08-04 against
  ...
  **Four patches are pending again:
  ...
  **[`0085`](../patches/0085-js-boxing-source-census.patch)** (where the boxes that remain are
  minted — `++` and `--`, not storage), on the usual terms: each push to the
  submodule remote returned 403, so the pointer is deliberately unbumped and every figure in their
  sections was measured on a local build of `07adeb44`. `0082` touches no file the other three do
  and **`0084` must follow `0083`, and `0085` must follow both**; each was verified to apply from a
  clean checkout of the pin with **`git am --keep-cr`** and the resulting tree diffed against the
  branch it was generated from, identical. Neither needs a main-repo fallback: `0082` is a compile-time change with a
