...
on the shape it targets; each one then meets the same gate. `CanScalarReplaceLocals` requires no
nested functions, no captured names, no `eval` and no `with`, and real code has those nearly
everywhere — which is the same gate that bounds phase 4's tiering candidates, found from the other
direction in 4-2.
...
| **3-6** | **Widen numeric-local eligibility** — *new, from 3-5's measurement* | `.Compiler` — `TryPlanScalarReplacement` | **5.0% of the Octane corpus's scalar locals reach the numeric tier.** That is the ceiling on 3-0, 3-3 and 3-5 alike — each correct, each large on its shape, each meeting the same gate. `CanScalarReplaceLocals` requires no nested functions, no captured names, no `eval` and no `with`, which real code has nearly everywhere; it is the same gate that bounds phase 4's tiering candidates (found from the other side in 4-2). **Count which conjunct costs the coverage before designing anything** | L |**Do not start 4-2 before 4-3 has a design.** Speculation without a mid-function
