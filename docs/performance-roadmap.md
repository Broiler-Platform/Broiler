...
| **3-5** | **A parameter cannot be a numeric local** — *new, from 4-5's control* | `.Compiler` — `NumericLocalAnalysis` | 3-3 excluded parameters and called it finished. The price was never taken: the same counted loop with a **parameter** bound runs at **33.77 ns and 32 B an iteration**, with a **literal** bound at **8.36 ns and 0 B** — same answer, **4.0x and a box per iteration**, because `i < n` compares a raw double against a `JSValue`. Copying the parameter into a local does not help; the local inherits its unknown type. `for (var i = 0; i < n; i++)` is the corpus's commonest shape. Needs 3-3's own dominance argument plus a guard for the arguments object's mapping. **This is where the call-path budget should go next** | M |
