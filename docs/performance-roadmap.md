...
- **`patches/` holds two open `Broiler.JS` patches again — `0103` and `0104` — and everything else
  this document measures is in the pin.** Item 3-1's conversion-site census (`0103`) and item 4-5's
  `out`-parameter frame (`0104`) were committed against the pin, verified to apply to it with
  `git am --keep-cr`, and **could not be pushed**: `Broiler-Platform/Broiler.JS` is outside this
