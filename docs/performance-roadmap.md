...
- **Provenance:** the pinned submodule pointer is **`cca39b4d`**, checked 2026-08-05 against
  ...
  **[`0090`](../patches/0090-js-numeric-local-defeat-tests.patch)** — the eight shapes that scope it
  to 3-8a, tests only. Usual terms: the push to the
  ...
  everything cleared above, **`0087` → `0088` → `0089` → `0090` is the required order**, and all four
  were verified by applying them in sequence to a clean checkout with
...
name a commit CI cannot clone. **Four patches are open again** (`0087`, item 3-1's order-preserving
guard placement; `0088`, the GC-pause denominator that prices it; `0089`, the update-target
census that re-opens item 3-8; and `0090`, the eight shapes that scope it to 3-8a), on the usual
403 terms.
...
### 3-8 · Guard a local's numeric-ness at run time — **0.36% was the wrong denominator; re-opened and scoped to 3-8a**
