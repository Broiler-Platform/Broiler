...
- **`patches/` holds ten open `Broiler.JS` patches — `0103`–`0112` — and everything else this
  document measures is in the pin.** Item 3-1's conversion-site census (`0103`), item 4-5's
  `out`-parameter frame (`0104`), item 3-1's root-box consumer split (`0105`) and the
  execution-weighted numeric-tier refusals (`0106`) the guarded-leaf read census (`0107`) and the
  consumer-side boxing-read census (`0108`) the element-read widening (`0109`) its root store arm (`0110`) the parameter population's read cost (`0111`) and the remaining populations (`0112`) were committed against the pin, verified to apply to it **in order** with `git am --keep-cr`, and **could not be pushed**: `Broiler-Platform/Broiler.JS` is outside this
  session's authorized repository set and the git proxy returns 403, which is the signal to fall
  back to a patch rather than to retry. **The pointer is therefore deliberately NOT bumped** — CI
  clones the submodule by pointer, and bumping it to a commit that was never pushed would break
  the build. Neither is listed in `scripts/apply-pending-wpt-patches.sh`: both are engine
  instrumentation and change nothing WPT renders. Read the pointer with `git submodule status`,
...
| Phase | State |
|---|---|
| **0** — evidence | 0-1…0-5 ✅, 0-9…0-11 ✅. **0-6 is now satisfied in both halves.** The workflow run has happened, its results are committed, and the run committed **2026-08-07** is the first that carries a **noise band**: `--repetitions 3`, median reported, a spread beside every score. **15 of 15 suites `ok` and 17 of 17 scores** for Broiler, Jint and a same-machine Chromium alike, nothing errored or timed out. Geomean **372** against Chromium's 58 718 (**158×**) and Jint's 577 (**0.644×**), spread **138×**. **16 of 17 scores are inside the declared 7.5% band**, median **3.0%**, the single exception EarleyBoyer at 7.9% — *and that overturns the local band this row used to quote*: the five suites a container put outside the band (Splay 15.9%, Crypto 12.5%, Richards 10.6%, SplayLatency 10.4%, DeltaBlue 9.1%) read **5.1%, 3.0%, 1.9%, 3.9% and 5.3%** on the runner. *A band is a property of a machine.* **Still not differenceable against its predecessor** — that one was single-repetition, and a banded run minus an unbanded one is an unbanded difference; the first claimable delta is this run against the next banded one. What is legitimate across all four runs is which side of a threshold each benchmark falls, and that is unchanged for a fourth time: Richards passes at **145×**, DeltaBlue fails at **512×**. **What remains owed in phase 0 is 0-7's BenchmarkDotNet comparison and 0-8's RID matrix**, neither of which a container can produce |
...
- **Coverage: 17 of 17 scores, all 15 suites `ok`, for all three engines** — Broiler, Jint and
  ...
  met, on all three runs.
- **The noise band is on record from the machine the gate closes on, and it holds.** Three
  ...
  flagged by the harness itself. **That is 0-6's remaining half, satisfied** — every committed
  result before this one disclaimed its own deltas because nothing in CI had ever passed
  `--repetitions`.
- **It also overturns the container band recorded below, and the correction runs the safe
  ...
  of a shared container and false of the gate machine, where the figure is nearer **2%**. The
  local table stays below, because the finding it actually supports is the one that survives:
  **a band is a property of a machine and does not transfer**, which is exactly why the gate
  names the RID matrix rather than a number.
- **A banded run and an unbanded one still cannot be differenced.** Between 2026-08-05 and
  ...
  `"Repetitions per suite: 1"`. **The first pair this campaign may difference is this run
  against the next banded one** — which is a thing that can now happen, rather than a thing
  that was owed.
- **What is still owed in phase 0 is 0-7's BenchmarkDotNet comparison and 0-8's RID matrix**,
  and a container cannot produce either.

#### The first band on record — taken in a container, and since superseded by CI's
...
and not a CI runner; what it was, was the first time anything in this campaign reported a
spread instead of promising one.

> **Superseded as a band, kept as a lesson.** The 2026-08-07 CI run measures the same thing on
> the gate machine and gets **16 of 17 inside 7.5%**, with all five of this table's offenders
> comfortably inside. **Do not quote this table's percentages as the campaign's noise floor** —
> quote CI's. What this one is still good for is the point it makes against itself: two honest
> three-repetition measurements of the same engine, days apart, disagree about the band by up
> to **5.6×**, so a spread is a property of the machine that took it.
...
Richards change under ~10% is not measurable at three repetitions"*, and CI's 1.9% says the real
figure on the gate machine is about a fifth of that. The conclusion §3.5 keeps is the narrower
one — a spread has to be re-measured wherever the arms are going to run, and a container is not
that place.
...
**The patch handoff, which was the third gate, has worked five clearings running** — every
`Broiler.JS` patch this campaign wrote from `0049` onward has been applied, pushed and the
pointer bumped, most recently a twelve-patch stack. **A 403 has come to mean *deferred* rather
than *stranded***; what makes that safe is that the pointer is never bumped locally, so nothing
here can name a commit CI cannot clone.

**Two corrections to what this paragraph used to claim, both read off the tree rather than
argued.** It said the pointer was `8308df51` and that
[`patches/README.md`](../patches/README.md)'s index carried no `Broiler.JS` row. The pin is
**`e5dc2610`**, and the index carries **ten** `Broiler.JS` rows — `0103` through `0112`, whose
files all exist in the pinned tree, i.e. the fixes are upstream and the backlog entries were not
retired with them. **They are left in place rather than deleted here**: the README's own rule is
that a patch is removed *once its fix is upstream*, and establishing that for a ten-patch
sequential stack means more than checking that its added files exist — a partially-applied stack
would look identical to this check. *Recording that the index is stale is the correct output of
noticing it; deleting a patch on a heuristic is not.* **What is true and load-bearing is
unchanged**: phase 5's item 2 is the only engine work currently held outside the pin, as
`patches/0115`.
...
**Regenerated by the workflow on 2026-08-07 at the pinned pointer** — `tests/octane/results/`,
