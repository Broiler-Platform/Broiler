...
- **Provenance:** the pinned submodule pointer is **`cca39b4d`**, checked 2026-08-05 against
  ...
  **One patch is pending again:
  ...
  it removes — on the usual terms: the push to the submodule remote returned 403, so the pointer is
  deliberately unbumped and every figure in its section was measured on a local build of
  `cca39b4d` plus that patch. It is independent of everything cleared above, applies directly to
  the pin, and was verified by applying it to a clean checkout with **`git am --keep-cr`** and
  diffing the result against the branch it was generated from — identical. It needs no main-repo
  fallback: `BROILER_JS_NUMERIC_TREE_ORDER=0` restores the previous emission exactly and is the
  bisection, and its refusal counters are touched once per compiled site rather than per call.
...
name a commit CI cannot clone. **One patch is open again** (`0087`, item 3-1's order-preserving
guard placement), on the usual 403 terms.
...
**What the wall clock says about all of it, though, should be read first.** Three measurements now
price allocation against time on this corpus — `0084` at 12.2% → 1.9%, this at 54.0% → 3.1% — and
they agree that removing boxes buys roughly a sixth to a seventeenth of their share in wall clock.
**A typed store that removed every remaining conversion would be worth about 2% of the driver on
that exchange rate.** It is worth having and it is not worth an XL ahead of anything with a better
ratio; whatever goes next in this phase should be argued against that number rather than against a
box count.
