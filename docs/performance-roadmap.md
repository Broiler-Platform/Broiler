...
- **Provenance:** the pinned submodule pointer is **`cca39b4d`**, checked 2026-08-05 against
  ...
  **[`0093`](../patches/0093-js-speculative-numeric-storage.patch)** — 3-8a's dual-representation
  local, built, off by default, and a measured regression until its read paths follow. `0090` and
  `0091` are tests only. Usual terms: the push to the
...
- **A speculation's losing side is every consumer you did not convert, and it can exceed the
  prize.** 3-8a's storage half does exactly what it was built to do — 835 584 update steps take a
  native double add and box nothing — and the corpus gets **2.1% MORE boxes**, because while the
  raw half is live every read that cannot consume a raw double mints one, and the shape reads its
  counter far more often than it increments it. *Before building a representation change, count the
  reads as well as the writes; a dual representation is only a saving once its consumers can take
  the cheap half, and until then it is a tax on every one of them.*
...
### 3-8 · Guard a local's numeric-ness at run time — **3-8a's storage half is built, and it is a regression until the reads follow**
...
- **3-9 (new, S–M, static, count first).** Import the enclosing function's proven-numeric set into
...
#### Built — the storage half — and it is a measured regression until the reads follow

The dual representation is built and is **off by default** (`BROILER_JS_SPECULATIVE_NUMERIC_LOCALS=1`).
A speculative local is held as a raw `double`, a `bool` saying the double is live, and the ordinary
`JSValue` slot; `Expression` becomes a conditional over the two, so **every existing read site is
correct without being touched** and a write through it is an assignment to a conditional, which the
backend rejects loudly. That is the numeric tier's own safety argument reused — the field it does
*not* get is `NumericStorage`, because five fast paths read that on the understanding that the
binding **is** a double, and a speculative one is a double only sometimes.

Writes route through `AssignToVariable`, which lands the value in the slot, derives the flag from
it and mirrors the raw half — branch-free, and reading the flag and the double **off the slot**
rather than off the expression, so a value with a side effect cannot run twice. The paths that want
the binding as an ordinary assignable `JSValue` (a compound assignment, a `with` fallback, a lexical
initializer) go through one escape hatch that materializes the slot, writes in place, and
resynchronizes. The `++`/`--` step branches on the flag: while it holds, the increment is a native
double add that **writes nothing back to the slot**, which is the box the census priced.
...
|---|--:|--:|--:|--:|
| Richards / DeltaBlue / RayTrace / Box2D | unchanged | 1.000 | 0 | unchanged |
| EarleyBoyer | 282 000 → 282 000 | 1.000 | 2 | 19 149 → 19 149 |
| Crypto | 13 412 826 → 13 412 808 | 1.000 | 1 | 7 192 946 → 7 192 910 |
| **NavierStokes** | **11 747 635 → 12 417 142** | **1.057** | **14** | **9 461 760 → 8 626 176** |
| **Total** | **31 162 965 → 31 832 454** | **1.021** | **17** | |

***It removes 835 584 steps and adds 669 507 boxes.*** The mechanism works — the steps really do
take the native arm — and the reads pay for it more than twice over: about **1.5 M boxes** appear
on `Expression`'s boxing arm, because while the raw half is live every read that cannot consume a
raw double has to mint one, and NavierStokes reads `currentRow` far more often than it increments
it (`x[currentRow]`, `x[currentRow-1]`, `x[++currentRow]`).

**So the storage half alone is not the item; the two consuming read paths are the rest of it.**
`0084`'s guarded arithmetic tree already carries exactly the right shape for a leaf — a raw double,
a flag and a fallback value are what its `OrderedNode` is — and item 3-0's
`GetElementByNumber(double)` already takes a raw double as an index. Neither has been made
guard-aware, and until both are, this is a 2.1% regression rather than a saving.
...
the same claim as the names holding the traffic*, and only running the arm distinguishes them. The
prize therefore falls from ≈115 ms to a tenth of that on the population as scoped, and what is left
of the item is mostly the read paths, whose benefit does not depend on the population at all.

**What is shipped is a correct mechanism with its consumer missing**, which is the same position
item 4-3b landed in and is recorded the same way: 1 171 compiler tests, 4 571 integration, 2 103
built-ins, all green **on both settings**, with `NumericLocalDefeatTests`' four shape fixtures now
Theories over the switch — the answer is unchanged and only the row moves, `LocalSlot` when off and
nothing when on, which is what says the speculation fires on exactly the shapes it was scoped from.

**What is established beyond the count.** `NumericLocalDefeatTests` carries the A/B the item rests
on, reduced to one difference:
...
Same nesting, same body, same update; one identifier different. So the enclosing-scope read is not
one defeat among several on this shape — **it is the defeat**, which is what 3-8a proposes to test
at run time. What is missing is only the count of how many such names the corpus has, and the next
attempt at that should start from the observation that killed this one: the drop-cause counter
aggregates across every function in the program, so it cannot answer a question about one.

***So the scoped item is an M for 0.6%, and that is the number it has to be argued with.*** It is
the best-founded item left in phase 3 — the population is named, the mechanism exists, the
soundness argument is one type test, and every competing conjunct has been ruled out by
measurement — and it is still half a percent. *Phase 3's remaining work is not blocked on a
missing idea; it is bounded by the exchange rate `0088` measured, and this is what that bound looks
like once an item is scoped properly instead of estimated.*
...
| **3-8** | **Guard a local's numeric-ness at run time** — *new, from 3-6's count* | `.Compiler` + 4-3b's `SpeculationBuilder` | The fixed point's **1 842 dropped candidates — 68% of all hoisted names** — are dropped for want of a *type*, not for want of a rule: the values come from parameters, property reads, elements and calls, none knowable statically. No widening of a conjunction reaches them. **The largest single obstacle in phase 3 is shaped like phase 4** — this is 4-3b's in-method branch pointed at a local's representation. **Do not start before 3-7 says how much the static half closes** | XL |bailout is either unsound or restricted to functions with no observable side effect
