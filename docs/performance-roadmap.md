...
- **Provenance:** the pinned submodule pointer is **`07adeb44`**, checked 2026-08-04 against
  ...
  **[`0083`](../patches/0083-js-arithmetic-operand-census.patch)** (item 3-1's first measurement —
  what the arithmetic operators are handed at run time), on the usual terms: each push to the
  submodule remote returned 403, so the pointer is deliberately unbumped and every figure in their
  sections was measured on a local build of `07adeb44`. They touch **no file in common**, so the
  order between them does not matter; both were verified to apply from a clean checkout of the pin
