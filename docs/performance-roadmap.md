...
**One shape is known to be missed and the corpus does not contain it.** A named function
*expression*'s own name is bound inside itself by the specification — `FreeNameScan` is right that
it is not free — and **this engine materialises that binding as a cell handed in to the lambda
anyway**. `var f = function g(n) { … g … }` reads `g/g predicted{}`. That is item `0097`'s lesson
for the third time — *ask what the compiler built, not what the analysis proved* — and the first
time it has decided a **mechanism** rather than a measurement. An attempt to add the self-name from
the function's own scope at prediction time did not resolve it, so the gap is **recorded rather
than papered over**, pinned by a fixture written to go off when it is closed. ***Zero on a corpus
is not zero***, and this is the instance that says so.

**State: the layout is validated, the deferral is not built, and the item is still L.** What is
now known that was not: the derivation never under-approximates on real source, it
over-approximates on 53% of sites, `ClosureRepository`'s two populations must be told apart, the
compiler-introduced captures need a reserved region, and one source shape is still unpredicted.
