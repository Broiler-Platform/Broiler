...
- **Provenance:** the pinned submodule pointer is **`14fa4f10`**, checked 2026-08-05 against
  ...
  **Seven patches are pending again, and four of them are one item.**
  ...
  **[`0107`](../patches/0107-js-generic-operator-ceiling.patch)** — item 4-2's arithmetic half
  ...
  at 0.475%**.
  And **[`0108`](../patches/0108-js-call-entry-cost.patch)** — item 4-5 **unblocked**: pricing the
...
empty for the second time since it was written. **Seven patches are open again** (`0102`–`0108`: item 1-1's remaining
...
arithmetic half priced and refused, and item 4-5 unblocked), on the usual 403 terms.
...
### 4-5 · The fixed cost of a call — **unblocked: 44% of a call entry is bookkeeping the engine's own short path already skips**
...
**The successor's first move is an ablation of those eight, not a profiler.** Each is reachable
from a benchmark or behind a switch, and the sum has a known target: 50.18 ns.
...
- **What is left of the item, ranked.** The whole `InvokeFunction` entry is **6.50%** of the corpus
  and the removable-looking half is **2.85%**; 4-4's ceiling is **2.43%**; 4-2's arithmetic half was
  refused at **0.119%**. **4-5 is where the phase-4 budget should go**, which is what 4-4's
  re-specification said and is now measured on the real corpus rather than on seven suites.
