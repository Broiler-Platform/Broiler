...
- **`patches/` holds nine open `Broiler.JS` patches — `0103`–`0111` — and everything else this
  ...
  consumer-side boxing-read census (`0108`) the element-read widening (`0109`) its root store arm (`0110`) and the parameter population's read cost (`0111`) were committed against the pin, verified to apply to it **in order** with `git am --keep-cr`, and **could not be pushed**: `Broiler-Platform/Broiler.JS` is outside this
