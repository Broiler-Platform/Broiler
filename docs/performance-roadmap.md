...
- **`patches/` holds three open `Broiler.JS` patches — `0103`, `0104` and `0105` — and everything
  else this document measures is in the pin.** Item 3-1's conversion-site census (`0103`), item 4-5's
  `out`-parameter frame (`0104`) and item 3-1's root-box consumer split (`0105`) were committed
  against the pin, verified to apply to it **in order** with `git am --keep-cr`, and **could not be
  pushed**: `Broiler-Platform/Broiler.JS` is outside this
