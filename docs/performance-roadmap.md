...
- **Provenance:** the pinned submodule pointer is **`cca39b4d`**, checked 2026-08-05 against
  ...
  **[`0098`](../patches/0098-js-blocking-host-wait.patch)**, which fixes the two deadlocks those two
  ...
  And **[`0099`](../patches/0099-js-free-name-scan.patch)** — item 1-1's named precondition, built
  ...
  everything cleared above, **`0087` → `0088` → `0089` → `0090` → `0091` → `0092` → `0093` →
  `0094` → `0095` → `0096` → `0097` → `0098` → `0099` is the
  required order**, and all thirteen were verified by applying them in sequence to a clean checkout with
  **`git am --keep-cr`** and building the result. Neither needs a main-repo fallback: `BROILER_JS_NUMERIC_TREE_ORDER=0` restores the
...
#### The remaining half, priced — and the price depends entirely on how the scan is written: **`0099`**
