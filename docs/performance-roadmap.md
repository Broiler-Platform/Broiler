...
- **Provenance:** the pinned submodule pointer is **`14fa4f10`**, checked 2026-08-05 against
  ...
  **[`patches/0102`](../patches/0102-js-deferral-population.patch)** — item 1-1's remaining half,
  ...
  available refused by the counter built to test it. And
  ...
  **7 of 15** suites it could reach, which moves item 4-1's headline from **93.54% to 80.11%** and
  ...
  host had it (§4.2a). Usual terms: the push to the submodule remote returned 403, so the pointer is
  deliberately unbumped and both sets of figures were measured on a local build of `14fa4f10` plus
  the patch in question. **They are independent of each other** — different files, no ordering
  constraint — though both were verified applying in sequence with `git am --keep-cr`. Neither needs
  a main-repo fallback: `0102`'s counter is off by default (`BROILER_JS_DEFER_TREE_COUNT=1`) and
  changes no emission on any setting, and `0103` touches only the benchmark host.
...
empty for the second time since it was written. **One patch is open again** (`0102`, item 1-1's
remaining half: its capture-free population counted at 728 sites of 5 762), on the usual 403 terms.
...
#### The mechanism itself: attempted, re-specified, **and not built**

With the layout validated, the next step was the deferral. It is **not built**, and the reason is
not the obstacle the item names.

**The item's stated raw material does not serve it, and that is provable rather than arguable.**
The item says `JSFunction` *"already carries `source` and already recompiles from it for tiering —
the raw material for deferring is present"*. Read, `RecompileForTiering` wraps the text as
`({source})` — or `'use strict';({source})` — and hands it to `CoreScript.Compile` **as a fresh
top-level script**. It has no enclosing scope by construction, which is exactly why 4-2a had to
refuse the identity cases and repair strictness. And the tiering gate enforces the same thing from
the other side: it admits a function only when `!cs.HasOuterFunctionCaptures && !cs.HasNestedFunctions
&& withBoundaries.Count == 0`, not an arrow, not a class, not direct-eval compilation.

***So the recompile path is sound precisely for the functions that have no enclosing context to
reproduce, which is the complement of the population a deferral serves*** — `0102` counted that at
**87.3% of sites capturing something**. The two mechanisms share a source string and nothing else.

**What the real obstacle is, itemised rather than gestured at.** A deferred body is compiled at
first call and must be handed everything `CreateFunction` would have had:

| Where it comes from | Count | Examples |
|---|--:|---|
| `CreateFunction` parameters carrying enclosing context | **14** | `super`, `createClass`, `className`, `memberInits`, `forceStrictMode`, `computedMemberNames`, `thisIsUninitialized`, `superConstructor`, `privateInstanceElements`, `autoAccessorBackingKeys`, `directEvalPrivateNames`, … |
| reads of the enclosing `FastFunctionScope` | **9** | `ThisExpression`, `NewTargetExpression`, `RootScope`, `SuperConstructor`, `DirectEvalPrivateNames`, `InParameterInitializer`, `GetVariable(name)`, `Parent`, `Function` |
| `FastCompiler` instance state | **5** | `pool`, `withBoundaries`, `IsStrictMode`, `inMemberInitializer`, `scriptInfo` |

**The capture layout — the one thing the item names — is now the *solved* part.** What is left is
reproducing twenty-eight other pieces of compiler state, each of which is a silent miscompile if it
is wrong, in code that only runs on first invocation and therefore only on the paths a test suite
reaches last.

**The design that follows from that, recommended rather than built.** *Do not snapshot the enclosing
scope — keep it alive.* A snapshot has to enumerate all twenty-eight and stays correct only while
nobody adds a twenty-ninth; holding the enclosing `FastFunctionScope` chain (declining to
`Dispose` it back to the `FastPool` while it has unforced children) reproduces the context by
construction and fails loudly rather than silently if something is missing. It trades the memory
the emission half already trades — §1-1 records that a deferred site retains its expression tree —
for a class of bug that cannot be tested out.

**Why it is not built here.** It is a change to the compiler's *lifecycle*, and its gate is the one
this document names for this item: full test262 over the five pinned manifests with **no new
failure and no new timeout**, plus the item's four spec-visible risks, plus generators, async and
direct eval. A half-verified version of this is not an instrument reading a wrong number — it is a
wrong answer in compiled code, on the least-exercised path in the engine. **Size: unchanged at L**,
and now with the L in a different place than the item says it is.
