...
- **Provenance:** the pinned submodule pointer is **`cca39b4d`**, checked 2026-08-05 against
  ...
  **[`0092`](../patches/0092-js-speculative-numeric-population.patch)** — 3-8a's population, counted
  at 26 names by an instrument made to discriminate first. `0090` and `0091` are tests only. Usual terms: the push to the
  ...
  everything cleared above, **`0087` → `0088` → `0089` → `0090` → `0091` → `0092` is the required
  order**, and all six were verified by applying them in sequence to a clean checkout with
...
name a commit CI cannot clone. **Six patches are open again** (`0087`, item 3-1's order-preserving
...
that survived the attempt to build it; and `0092`, its population counted at 26 names), on the
usual 403 terms.
...
### 3-8 · Guard a local's numeric-ness at run time — **re-opened, scoped to 3-8a, and its population counted at 26 names**
