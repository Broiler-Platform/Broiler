...
- **Provenance:** the pinned submodule pointer is **`14fa4f10`**, checked 2026-08-05 against
  ...
  **Eight patches are pending again, and four of them are one item.**
  ...
  **[`0108`](../patches/0108-js-call-entry-cost.patch)** — item 4-5 **unblocked**: pricing the
  ...
  And **[`0109`](../patches/0109-js-call-bookkeeping-attribution.patch)** — that bookkeeping
  **attributed**, with the sum closing to within 0.5 ns: **92% of it is Annex B `caller`/`arguments`**
  at 44.40 ns per call to every ordinary non-strict function (**≤1.65% of the corpus**), while the
...
empty for the second time since it was written. **Eight patches are open again** (`0102`–`0109`: item 1-1's remaining
...
arithmetic half priced and refused, and item 4-5 unblocked and attributed), on the usual 403 terms.
...
| **the legacy caller/arguments frame** — upper bound, JS-callee calls only | **≤1.65%** |
...
**The legacy frame is the item**, and it is bounded rather than measured: 44.40 ns is charged only
to calls whose callee is an ordinary non-strict *function*, so arrows, methods, class bodies, strict
functions and native builtins are all outside it, and **≤1.65%** is a ceiling that the true share
sits under by however much of the corpus those account for. **The next measurement is that split**
— one counter beside `HasLegacyCallerArguments` — and it costs a line.
