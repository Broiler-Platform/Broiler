...
- **Provenance:** the pinned submodule pointer is **`cca39b4d`**, checked 2026-08-05 against
  ...
  **Two patches are pending again:
  ...
  it removes — and **[`0088`](../patches/0088-js-gc-pause-accounting.patch)** — the GC-pause
  denominator that prices it, and phase 3, for the first time. Usual terms: the push to the
  ...
  everything cleared above, **`0087` → `0088` is the required order** (both edit the benchmarks
  driver), and both were verified by applying them in sequence to a clean checkout with
...
name a commit CI cannot clone. **Two patches are open again** (`0087`, item 3-1's order-preserving
guard placement, and `0088`, the GC-pause denominator that prices it), on the usual 403 terms.
...
- **The `++`/`--` step is the cheaper bid and should be counted before the store is built.** It is
  33.2% of remaining requests against the conversion's 47.4%, it is concentrated on NavierStokes
  (9.46 M) which is the corpus's highest-rate boxer, and — unlike a typed store — nobody has yet
  counted *where its operand lives*. If a large share are element or field updates, it shares the
  storage mechanism; if they are locals the analysis failed to prove numeric, it is a much smaller
  change. That count is an afternoon and it is the same "measure the premise first" discipline that
  has now retired four designs in this phase.
