...
**What is *not* established, and is recorded that way deliberately.** With the comparison corrected,
the corpus run reports **170 missed sites of 5 762 and 6 138 missed names** — but the same run
reports Mandreel at **2 622 exact against 1 476 predicted sites**, i.e. more checks than
predictions, so the per-site figures are **double-counting somewhere and are not trustworthy yet**.
The missed-name samples show at least three distinct unexplained shapes — top-level `var`s read
from a top-level function (`predicted{}`), a named function expression's own name (Box2D's
`F/F predicted{this}`), and jQuery's minified nesting. *A miss rate from an instrument that
disagrees with itself is not a finding*, so no go/no-go is claimed here and the item is **not**
declared blocked on the strength of it.

**State: the checker is built, green on nine constructed shapes, and reads non-zero on the corpus
for reasons not yet isolated.** The next step is the double-count, then the three sample shapes —
in that order, because the second cannot be read while the first is live. The deferral mechanism
itself remains unbuilt and remains **L**.
