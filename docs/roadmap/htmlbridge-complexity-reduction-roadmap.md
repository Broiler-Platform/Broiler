...
closed; the remaining Phase-7 item-6 work is a **bridge application task** — wire `DomBridge` to drive the
engine module path and retire the `EsModuleLinker` (plus real event-loop ordering) — not a `Broiler.JS` gap.
Phase 8 remains proposed.
