...
| 3 — feature modules | **Complete** | All four exit criteria are met (verified 2026-07-21): ~60 feature-binding modules extracted with co-located tests and declared host contracts; the `DomBridge.cs` facade is **within** the 500–800-line target (701 lines) — the earlier "still to come" list (Element/geometry — P3.51, SVG — P3.50, Window/Document, Canvas) is done; and the 750-line file-size ratchet is fully closed (debt list empty, guard `HtmlBridgeArchitectureGuardTests`). Work item 6 — externalize the embedded content-rendering polyfill JS as a versioned asset — is done (P3.53). **The last residue — the Phase-6-gated Canvas `getContext`/2D callbacks — is now closed (P3.64, 2026-07-23):** with Phase 6 / Phase 8 F1 (P8.9) dissolving `Broiler.HtmlBridge.Rendering` into `Dom`, the `CanvasCommandRecorder` coupling that pinned the callback in the mixed `JsObjects.cs` file is gone, so `getContext` + `BuildCanvas2DContext` + the `JsUtilities…034…058Core` drawing callbacks were extracted into the co-located `Dom.Features.CanvasBinding` module (`CanvasContextBindingTests` green). Every element-member callback is now a co-located feature module. |
...
| 5 — used-value behaviour into Layout | **In-scope terminal** | Anchor-track deletion complete through step 6 (verified 2026-07-21). Feature (b) visual-viewport/zoom: read (CSSOM) **and** render (pixel) sides validated on the engine used-value model — `NativeVisualViewport` is on, the zoom-serialization revert machinery is deleted, and `NativeZoom`/engine-render reftests pass; deleting the zoom bake is now only a *deployment* gate (enable `NativeZoom` at the external `CaptureService` renderer). Feature (a) dialog: native modal centering wired, modal box-chrome bake deleted, **and the native
`::backdrop` box + top-layer paint cutover landed (2026-07-21)** — those submodule pieces
(`PaintWalker` top-layer paint + `DomParser.GenerateNativeBackdrops`) are applied and pinned, so
`DomBridge.NativeTopLayer`/`NativeBackdrop` now default on (the bridge stamps native markers; the z-index /
`<div>` emulation is retained only as a gated fallback for author position-try backdrops, the WPT-lever, and
rollback), validated by `NativeBackdropRenderTests` end-to-end pixels. The residue is externally gated: the
pinch-zoom render cutover (patch `0001` API applied+pinned, but dropping the bake needs the out-of-scope
external renderer to accept `viewportZoom` + cross-boundary scale plumbing, with no pinch reftest); the
...
| 6 — remove Broiler.HtmlBridge.Rendering | **Complete** | All three concerns delivered; submodule patches `0004`–`0007` applied upstream and the `Broiler.HTML` pointer pins them (`5c16c12`); the `HtmlPostProcessor` video/progress/meter/select fallbacks are dropped. The terminal **`Broiler.HtmlBridge.Rendering` project deletion is done** (2026-07-22, via Phase 8 follow-up F1 / P8.9): the project is deleted and its sole occupant, the internal `HtmlPostProcessor`, was folded into `Broiler.HtmlBridge.Dom`. The test-harness shims (`Process`/`Strip*`/`RewriteRootSelector`) moved with it and are still slated for eventual retirement behind the external WPT pixel reftest gate, but the assembly itself is gone. |
| 7 — isolate loading/security/browsing-context policy | **Items 1–5 complete; item 6 engine path active** | CSP split, script descriptors, one shared `UrlResolver`/`Origin`, external-stylesheet CSP and host-layer enforcement all landed (verified 2026-07-21). Item 6: the static import/export graph is linked+executed with `import.meta`, dynamic `import()` and scope-accurate live bindings — modules are not silently skipped. The engine-coupled tail is **closed and active**: the host-resolution seam (patch `0008`), the top-level-await **codegen** fix (`0010`, P7.25) and the **module-orchestration completion** fix (`0011`, P7.26) are **all applied upstream and pinned** (`Broiler.JS` → `98b07636`), so the engine module machinery binds a static import's value (all full-suite-validated, zero regressions). The **bridge is wired to the engine module path, capability-gated** (P7.27, `BridgeModuleContext` + `EngineModuleSupport` probe): `ScriptEngine` runs the authorised `ModuleRoots` through the engine when it binds imports, else the demoted `EsModuleLinker` fallback. **On the pinned engine the probe now returns `true`** (re-verified 2026-07-22 by running the probe directly; 303 module/snapshot/boundary tests pass on the engine-driven path), so the engine-driven path is the active one. **Tail in delivery** (see the [module-execution consolidation record](htmlbridge-module-execution-consolidation.md)): increments 1–2 done (P7.28 — the engine-module seam `BridgeModuleContext`/`EngineModuleSupport` relocated from `Scripting` down into `Dom`; P7.29 — the iframe path `ExecuteSubDocumentScripts` migrated onto the engine `ModuleRoots` path, engine-gated with linker fallback, closing the one un-gated `ModuleScripts` consumer; P7.30 — `CaptureService` gained engine module support, additive since capture dropped modules before; P7.31 — the `EsModuleLinker` and its scanner/linker/loader/wrapper deleted (~1,900 lines), `ScriptExtractionResult.ModuleScripts` removed, every surface now engine-only). **The module-execution tail is closed** — all script-execution surfaces run ES modules through the engine's own machinery. Real event-loop ordering is a larger `BrowserEventLoop` rework delivered as its own increments: EL-1 done (P7.32 — deadline-ordered timeouts: the delay is plumbed through and timeouts fire earliest-deadline-first with a FIFO tiebreak, vs. the previous unordered per-step snapshot); EL-2 done (P7.33 — deadline-ordered intervals: `setTimeout`/`setInterval` unified into one deadline-ordered timer queue, so an interval ticks by its period interleaved with timeouts instead of once per drain step); EL-3 done (P7.34 — the `ScriptEngine` render pipeline drains only microtasks between the synchronous script phases and defers timers to after the window load event, so timers fire in deadline order after all script execution rather than eagerly between scripts; P7.35 — the same deferral applied to the CLI-capture path `CaptureService`, verified Acid3-score-unchanged). Remaining: one ordered queue interleaving deferred/module tasks with timer tasks (vs. fixed phase buckets). Real event-loop ordering is scoped separately as a larger `BrowserEventLoop` rework. |
| 8 — simplify Core and Scripting | **Complete** | Items 1–6 delivered/decided (P8.1–P8.8) and all three assembly-decision follow-ups landed — F1 (P8.9: `Rendering` dissolved into `Dom`), F2 (P8.10: scripting mechanism carved into `Broiler.HtmlBridge.Internal.Scripting`), F3 (P8.11: the last two public mechanism helpers `internal`-ized). The `Core`-purity exit criterion is met and the final three-assembly shape (Core/Dom/Scripting) is realized; see the [assembly decision record](htmlbridge-assembly-decision.md). Detail below: Item 3 **complete** (P8.1/P8.2): async-drain-limit exhaustion is an explicit diagnostic across all three drain loops. Item 2 **complete** (P8.3): every `InteractiveSession` owns a private event-loop/context lifetime — `Dispose` tears down the bridge (event loop) then the context, and a failed `ExecuteInteractive` disposes both instead of leaking. Item 5 **complete** (P8.4): the misnamed `IScriptExtractor.cs` (no such interface; extraction is the static `ScriptExtractionService`) renamed to `ScriptExtractionResult.cs`, dangling doc cref fixed. Item 1 **complete** (P8.5 + P8.6, 2026-07-22): `IScriptEngine` segregated into `IScriptExecutor`/`IInteractiveScriptEngine`/`IScriptProfiling`/`IScriptEventLoop`, aggregated by the unchanged v2 `IScriptEngine` adapter (P8.5); and `ScriptEngine`'s render/typed and interactive bodies consolidated onto one shared `RunPageScripts` pipeline parameterised by a drain delegate (P8.6) — the *"one execution pipeline shared by normal/detailed/typed/interactive entry points"* exit criterion is met. Item 4 **consistency delivered** (P8.7, 2026-07-22): profiling funnels through one `RunMeasured` seam so an attached `ScriptProfilingHook` sees every script — `inline-{i}`, `deferred-{i}`, `module-{key}` — not just the inline ones; a tree sweep found no real consumers, but full removal is a v2-unadaptable surface deletion, so the keep-vs-trim-to-host-diagnostics call is handed to item 6's assembly/surface decision. Item 6 **decided** (P8.8, 2026-07-22): the [assembly decision record](htmlbridge-assembly-decision.md) sets the final shape at three code assemblies — `Core` (shared kernel), `Dom` (Web API bindings), `Scripting` (host) — folding the one-file `Rendering` assembly-per-feature artifact into `Dom`, keeping `Core`'s shared mechanism in one assembly behind a named internal namespace (not a fourth assembly), keeping the static `RenderLogger` as a sanctioned diagnostics primitive, and **keeping** the now-consistent profiling hook (v2-adaptation rules against a v3-only deletion). The `Core`-purity exit criterion's physical moves (F1–F3) are sequenced as verified follow-ups. See [remaining phases](htmlbridge-complexity-reduction-remaining.md). |
