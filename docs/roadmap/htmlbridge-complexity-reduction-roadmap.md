...
| 8 — simplify Core and Scripting | **Started** | Item 3 **complete** (P8.1/P8.2): async-drain-limit exhaustion is an explicit diagnostic across all three drain loops. Item 2 **complete** (P8.3, 2026-07-22): every `InteractiveSession` owns a private event-loop/context lifetime — `Dispose` tears down the bridge (event loop) then the context, and a failed `ExecuteInteractive` disposes both instead of leaking. Items 1, 4, 5, 6 remain. See [remaining phases](htmlbridge-complexity-reduction-remaining.md). |
