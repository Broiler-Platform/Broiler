...
dynamic `import()` (P7.20) and live bindings (P7.22) handled at the bridge layer, leaving item 6's
engine-coupled tail (top-level-await-as-async with event-loop ordering, and fully general live named
bindings). The `Broiler.JS` seam for driving the engine's own module machinery ships as patch `0008` (P7.19)
...
| 2 — document services & single state authority | **Complete (in-scope)** | All in-scope work landed: the process-static per-element runtime tables are **fully de-globalized** to per-bridge instances — `ElementRuntimeState`/`GetElementRuntimeState` no longer exist (the last concern, inline-style, was moved and the type renamed `InlineStyleRuntimeState`), `PositionAreaResolutions` is a per-bridge instance table, and a boundary guard enforces "no process-static per-element table" (12/12 green, verified 2026-07-21). The only residue — two *simultaneous* sessions cannot see each other's state — is **out of scope**: it is blocked below the bridge at the Broiler.JS engine's process-static current-context model (`CoreScript.GetCurrentContext`; last context wins), a JS-engine concern needing engine work, not a bridge change. |
