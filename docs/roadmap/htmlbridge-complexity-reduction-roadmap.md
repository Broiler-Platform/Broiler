...
top-level-await **codegen** bug that gated the engine-driven path (root-caused P7.21, proven P7.24) is now
**fixed by patch `0010` (P7.25)** — the generator box-load prologue re-seeded the persisted `ScriptInfo` box
with a null body-local on every resume, clobbering its `Indices` table; guarding the seed on `nextJump == 0`
fixes it, **full-`Broiler.JS`-suite-validated with zero regressions** (push 403 → ships unbumped). The one
remaining engine-coupled piece is the separate **module-orchestration completion** fix (§0008), without which
a static import binds `undefined`/`0` (no crash), so the bridge keeps its own `EsModuleLinker`. Phase 8
remains proposed.
...
| 7 — isolate loading/security/browsing-context policy | **Items 1–5 complete; item 6 delivered at the bridge layer** | CSP split, script descriptors, one shared `UrlResolver`/`Origin`, external-stylesheet CSP and host-layer enforcement all landed (verified 2026-07-21). Item 6: the static import/export graph is linked+executed with `import.meta`, dynamic `import()` and scope-accurate live bindings — modules are not silently skipped. Residue is the engine-coupled tail (top-level-await-as-async + real event-loop ordering): the host-resolution seam (patch `0008`) is applied upstream and the top-level-await **codegen** bug is now **fixed by patch `0010`** (P7.25, full-suite-validated, pending push). The one piece left is the separate **module-orchestration completion** fix (§0008) so a static import binds its value; until then the bridge keeps its `EsModuleLinker`. |
