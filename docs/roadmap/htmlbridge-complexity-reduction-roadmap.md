...
| 8 — simplify Core and Scripting | **Started** | Item 3 **complete** (P8.1/P8.2, 2026-07-22): async-drain-limit exhaustion is now an explicit diagnostic (`ScriptEngine.AsyncDrainLimitExhausted` + warning) instead of a silent stop, across all three drain loops (`ScriptEngine`, `CaptureService`, `WptTestRunner`). Items 1, 2, 4, 5, 6 remain. See [remaining phases](htmlbridge-complexity-reduction-remaining.md). |
