...
- Triage tooling fix: `scripts/run-wpt-tests.sh` now anchors its skipped-count extraction on the final `Results:` line so the periodic `[INFO] Completed ...` progress checkpoints stop polluting `tests/wpt-results/wpt-summary.txt` with hundreds of bare-number lines and the reported `Skipped` total now matches `wpt-results.json`.
