...
**Status:** Phase 3 has started; representative `calc-*`, viewport-aware media-query resolution, `ch` font-relative lengths, padded client/scroll metrics, zoom rendering, zoom-sensitive CSSOM view guard rails, and zoomed `scrollIntoView()` spacing/absolute-position cases now pass locally, including client/offset metric and offset-parent follow-up coverage, but the broader Phase 3 buckets still need follow-up triage.
