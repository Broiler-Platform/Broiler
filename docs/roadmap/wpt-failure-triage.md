...
- Phase 6 has widened again: focused `Broiler.Wpt.Tests` now mirror the upstream `checkbox` / `radio` vertical baseline optional cases for `vertical-lr` and `vertical-rl`, so the reopened `css/css-writing-modes/forms` checkable-control alignment slice is guarded alongside the existing range/button/select coverage.
- Phase 6 has widened again: live `progress` / `meter` elements now serialize into simple native-like fallback tracks with writing-mode-aware fill direction, so focused CLI and WPT guards cover the remaining `css/css-writing-modes/forms/meter-appearance-native-*` follow-up slice locally.
- Phase 6 has widened again: static `select[multiple]` rendering now rewrites into simple native-like listbox placeholders that distinguish horizontal, `vertical-lr`, and `vertical-rl` block flow and keep `appearance:none` visually distinct from native chrome, with focused CLI and WPT guards covering the remaining `css/css-writing-modes/forms/select-multiple-*` follow-up slice locally.
- Phase 6 has widened again: selector matching now lets `:is(...)` / `:where(...)` inside the focused `:has(...)` invalidation path evaluate complex sibling and `:nth-*` selector-list arguments instead of being stripped as unknown pseudos, with focused CLI and WPT guards covering the remaining `css/selectors/invalidation/is-pseudo-containing-sibling-relationship-in-has.html` follow-up slice locally.
- Phase 6 has widened again: default `<summary>` rendering now uses a simple disclosure marker that flips with the enclosing `<details open>` state, and focused selector rendering/WPT guards cover the remaining `css/selectors/selectors-4/details-open-pseudo-*` follow-up slice locally.
- Phase 6 has widened again: `:lang(...)` selector matching now rejects invalid language ranges (for example `:lang(0)` or mixed lists containing `0`) instead of letting later valid-looking entries match, with focused CLI and WPT guards for the invalid-range `selectors-4/lang-*` follow-up slice.
- Phase 6 has widened again: computed-style and auto-size resolution now short-circuit in-progress lookups, so sibling-change `:nth-child(... of .c)` / `:nth-last-child(... of .c)` selector invalidation cases can execute without recursive stack overflows, with focused CLI and WPT guards for that remaining `css/selectors/invalidation/nth-*` follow-up slice.
- Phase 6 has widened again: focused selector matching now supports the `:has(...)` relative-selector forms used by the remaining `has-with-nth-child*` invalidation cases, including descendant, child, adjacent, and general-sibling walks combined with nested `:nth-child(...)` / `:nth-last-child(...)`, with focused CLI and WPT guards for that `css/selectors/invalidation/has-with-nth-child*` follow-up slice.
- Phase 6 has widened again: `offsetTop` / `offsetLeft` now include the offset parent padding edge for in-flow descendants while still excluding borders, with focused CLI and WPT guards covering the `css/cssom-view/offsetTopLeft-border-box.html` display-matrix follow-up slice.
...
- Phase 6 has widened again: `Range.getBoundingClientRect()` / `getClientRects()` now expose a minimal client-rect path for fully-contained range nodes and recurse through `display:contents` descendants, while normal-flow sibling layout now lets `display:contents` descendants contribute their block extent to later siblings, so the focused `css/cssom-view/range-bounding-client-rect-with-display-contents.html` follow-up behavior is covered locally with a dedicated CLI guard.
...
- Phase 6 has widened again: renderer-side logical `border-inline-*` / `border-block-*` longhands and shorthands now map into the physical border paint path for the default writing-mode cases, with a focused `Broiler.Wpt.Tests` guard rail covering the visited-context logical-border slice inside `variable-reference-visited.html`.
- Phase 6 has widened again: renderer-side `text-decoration-line/color` longhands plus `text-decoration` shorthand now feed underline paint color through custom-property substitution, with a focused `Broiler.Wpt.Tests` guard rail covering the visited-context text-decoration slice inside `variable-reference-visited.html`.
- Phase 6 has widened again: the inline SVG renderer now parses `rgb(...)` / `rgba(...)` presentation colors instead of treating them as unsupported names, with a focused `Broiler.Wpt.Tests` guard rail covering the SVG paint-color path that still shows up in the remaining `css/css-variables` visited-context slice.
...
- Phase 6 has widened again: bridge-side approximate button and `input[type=button|submit|reset]` computed sizing now expands the logical block axis for multiline labels while keeping the inline axis stable, so the native-appearance writing-mode slice grows in height for horizontal controls and width for vertical controls; focused CLI and WPT guard rails now cover the `button-appearance-native-computed-style.html` opening slice locally.
- Phase 6 has widened again: `select[multiple]` and `select[size]` now use listbox-style logical block sizing in bridge computed styles, and DOM scroll metrics treat those controls as block-axis scrollers across horizontal, vertical, and sideways writing modes; focused CLI and WPT guard rails now cover the `select-multiple-*` / `select-size-scrolling-and-sizing.optional.html` slice locally.
- Phase 6 has widened again: subframe root/window scrolling now measure iframe viewport size from either CSS sizing or `width`/`height` attributes, and root scroll extents walk nested descendants instead of stopping at an auto-sized `<body>`, so the focused `scrollIntoView()` subframe root/fixed-target bucket scrolls the intended iframe and outer containers again; focused WPT guards now cover the `scroll-behavior-subframe-root.html` / fixed-iframe follow-up slice locally.
...
- Phase 6 has widened again: cssom-view hit-testing now gives SVG text content elements coarse boxes from their text anchors, direct text runs, font size, and simple `textPath` start points, so focused `elementsFromPoint-svg-text.html` cases can return `<text>`, `<tspan>`, and `<textPath>` targets instead of collapsing them back onto the outer SVG root; focused CLI and WPT guards cover that text-content follow-up slice locally.
- Phase 6 has widened again: cssom-view hit-testing now gives simple HTML table cells coarse grid boxes from the table’s own rect plus border-spacing, direction, and vertical writing-mode, while suppressing intermediate row/section boxes in hit results, so focused `elementsFromPoint-table.html` cases can return the expected `<td>` targets and fall back to the table for inter-cell gaps; focused CLI and WPT guards cover that table-layout follow-up slice locally.
- Phase 6 has widened again: cssom-view hit-testing now lets supported image-map `<area>` shapes win over their associated `<img>` when the hit point falls inside the mapped geometry, while points outside the mapped area still fall back to the image itself; focused CLI and WPT guards cover that `elementFromPoint.html` image-map follow-up slice locally.
- Phase 6 has widened again: cssom-view hit-testing now excludes clipped rounded-corner regions from border-box hits, so rounded fieldsets and similar boxes no longer win `elementFromPoint()` in their transparent corner cutouts while still hitting through the center; focused CLI and WPT guards cover that `elementFromPoint.html` fieldset follow-up slice locally.
- Phase 6 has widened again: cssom-view hit-testing now extends outside list-item marker regions back to their owning `<li>`, including simple text markers and list-style images, so `elementFromPoint()` can still resolve the list item when the hit lands on an outside marker instead of only inside the content box; focused CLI and WPT guards cover that `elementFromPoint-list-001.html` follow-up slice locally.
