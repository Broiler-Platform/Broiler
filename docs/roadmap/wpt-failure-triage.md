...
**Status:** Phase 3 has started; representative `calc-*` work now includes deeper parenthesis stacks, multi-argument `max()` lists, invalid unitless zero rejection in math functions, negative `calc()` media-query range clamping, and focused viewport `calc()` geometry resolution in the bridge, alongside viewport-aware media-query resolution including `vmin` / `vmax`, `ch` / `ex` / `lh` / `rlh` font-relative lengths, padded client/scroll metrics including mixed-zoom overflow, scroll API alias/options and writing-mode-aware clamp behavior, zoom rendering, zoom-sensitive CSSOM view guard rails, and zoomed/fixed-position `scrollIntoView()` spacing/absolute-position/percentage-inset cases that pass locally; however, the broader Phase 3 buckets still need follow-up triage.
