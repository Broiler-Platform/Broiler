...
| 37 | **`css-grid` "top-10 biggest problems" tail (#1248).** After cluster 36 fixed the shared `vertical-rl` container-placement bug, the remaining nine listed failures each need a distinct grid feature, not a bug fix: grid-axis transposition for vertical writing modes (the vertical alignment cluster, ~59 tests), auto-fill track count under shrink-to-fit + min-size, abspos items spanning implicit tracks, gutter track/margin accounting, subgrid across an orthogonal flow, and the experimental `grid-lanes` named-line auto-fill. Full per-item root cause, proposed approach, risk, and validation captured in a dedicated roadmap. **Workstream B (item #5) ✅ fixed this session:** the earlier "~16 % horizontal offset" premise was wrong — reproducing the test in-sandbox showed the `block-size:55vw; aspect-ratio:2/1` `<img>` **collapses to height 0** inside the nested `display:grid` (blank render). The implicit-only grid pass declined for *any* replaced item and the fallback approximation dropped the height; even engaging the real pass left the row measuring the box's stale `ActualBottom=0` (a line-box-measured replaced element records its height on its image word). Fixed by admitting a **definite-block-size** `<img>` to the real pass (`GridImplicitPathItemsAreSimple`) and sourcing the row size + `PlaceItemInArea` height from that definite block-size (`GridReplacedItemDefiniteBorderBoxHeight`) — the test now lays out identically to `-ref.html`. Guard `NestedGridItemBlockSizeTests`; 0 regressions on vendored css-anchor-position (40) + css-align (28) + grid/aspect-ratio Cli.Tests. **Workstream C (item #9) advanced this session:** scored `grid-auto-repeat-min-size-001`'s 12 checkLayout variants in-sandbox and fixed two more root causes — a `min-height`/`max-height` clamp on a **float's explicit height** (a general float bug: `height:100; min-height:200` kept 100) and **intrinsic-sizing width keywords** (`min-content`/`max-content`/`fit-content` fell through to the 1024 container width; now shrink-to-fit per CSS Sizing 3 §5.1) — taking the test **2/12 → 9/12** cases correct. **Now 12/12 — the 3 `box-sizing:border-box` variants closed this session:** the auto-fill repetition count under a definite *min*-size floored (largest count that does not overflow) when §7.2.3.2 requires the *smallest* count that reaches the min (ceil) — invisible for the g1–g8 clean-multiple min-sizes but off-by-one for a border-box content min-height of 200−20=180 (⌈180/50⌉=4, not 3); fixed by threading a `fillMinimum` flag (set only when the block size came from `min-height` with no definite height, so a definite explicit height still floors) into `ExpandAutoRepeatTrackList`. And `height:min-content`/`max-content` under `border-box` was reinterpreted as a specified border-box length, dropping the 20px border (offsetHeight 200 vs 220); fixed by excluding an intrinsic-sizing *height* keyword (`IsIntrinsicSizingHeightKeyword`) from the two explicit-length-height branches in `CssBox`, so the content-derived height stands and only the §10.7 clamp applies. Guard `GridAutoRepeatMinSizeTests` (all 12 variants) + corrected `GridTrackLayoutTests.GridAutoFillRows_ResolveCountFromMinHeight` (it had encoded the old floored count); 0 regressions across vendored css-anchor-position + css-align + css-backgrounds (byte-identical pass sets 30/40, 19/28, 39/61). **Workstream E (item #7) advanced this session:** the `grid-gutters-and-tracks-001` gutters were missing because Broiler never mapped the legacy CSS Grid Level 1 gap aliases — `grid-gap`/`grid-row-gap`/`grid-column-gap` were dropped (only the modern `gap`/`row-gap`/`column-gap` were recognised), so every track past the first abutted with no gutter. Aliased them in `CssUtils` (guard `GridGapAliasTests`, 0 regressions); remaining E sub-grids are blocked on named-line track support, grid-track-based `fit-content` width, and percentage-track sizing. **`fit-content()` *track* sizing added this session:** the `fit-content(<length-percentage>)` function (declined outright before → single-column approximation) now sizes to `max(min-content, min(L, max-content))` (§7.2.3); it required a prerequisite core fix — `CssBoxHelper.GetMinMaxSumWords` reset the max-content line for an inline-block child, so sequential inline-blocks measured as the *widest* not the *sum* (two 40px inline-blocks → 40, collapsing every `max-content`/`fit-content` track holding them), now accumulated as the inline-level content they are. The companion shrink-to-fit path (`CssBox.ComputeShrinkToFitWidth`, used by floats/inline-blocks/abspos) had the same defect (it summed adjacent floats but reset the line for an inline-block) and was fixed via the shared `CssBoxHelper.IsAtomicInlineLevel` predicate (guard `ShrinkToFitInlineBlockRunTests`). Guards `GridFitContentTrackTests` (two-inline-block construction: min-content 40, max-content 80, `fit-content(60)`→60, `fit-content(30)`→40), 0 regressions across the vendored css-align/css-anchor-position/css-backgrounds/CSS2 subsets (table/multicol `GetMinMaxWidth` consumers are unvendored → CI-gated). **Workstream D (item #4) fixed for LTR this session:** `grid-positioned-items-within-grid-implicit-track-001` needed **leading implicit tracks** — a definite line resolving *before* the explicit grid (a negative index) references an implicit track ahead of it (CSS Grid §8.3), but `ParseSingleGridLine` clamped those to `auto`, so the in-flow `-5/5` reference item collapsed to one cell and every abspos item (whose geometry depends on the leading tracks) filled the whole CB. Implemented leading-track normalisation for in-flow items (`explicitStart` shift + `ResolveTrackSizes` offset) and dedicated abspos line-to-area resolution (`ParseAbsposGridLines`/`ResolveAbsposAxis`, `auto`→padding edge per §9.2); the full LTR test passes **64/64** in the harness (guard `GridAbsposImplicitTrackTests`, 0 regressions across the vendored subsets). **Workstream A (items #6/#8/#10) re-scoped this session:** probing `vertical-rl` grids through the harness disproved the roadmap's "transposition fundamentally broken / multi-day rewrite" premise — placement, content distribution, and self-alignment all produce **spec-correct transposed geometry** (guard `GridVerticalWritingModeTests`). The real remaining vertical-grid gaps are narrower standalone features: **grid-track-based `fit-content` width** and **baseline self-alignment** (#6). **The fit-content grid width is now ✅ done for horizontal grids this session:** a grid with a fixed column template sums its column tracks (+ gaps + padding/border) for `min-content`/`max-content`/`fit-content`/`float` (`TryComputeGridIntrinsicContentWidth` hooked into `ComputeShrinkToFitWidth`/`GetMinMaxWidth`), instead of collapsing to its inline content — plus a frame-mismatch fix in the C fit-content branch. Unblocks E's test 1a and #10's container **on a real viewport (CI)**; the harness's zero `available` collapses the min≠max fit-content case, and vertical writing modes are gated off (rows-axis mapping through the rotation is a separate increment). Guard `GridIntrinsicWidthTests`, 0 regressions (55 grid guards + vendored subsets byte-identical). **E's named-line group (tests 5/6/12) fixed this session:** the blocker was placement, not track sizing (`ParseTrackTokens` already skips `[name]` brackets) — Broiler dropped the `grid-area` shorthand + `grid-*-start`/`grid-*-end` longhands (so `grid-area:3/3` auto-placed), and `ParseSingleGridLine` mapped named lines to `auto`. Added `grid-area`/longhand expansion (`CssUtils`) and named-line resolution (`ParseLineNames` → per-axis name→index map, threaded into in-flow + abspos placement). Guards `GridPlacementShorthandTests` + `GridNamedLineTests`, 0 regressions across the vendored subsets. **RTL grid mirroring added this session:** the inline (column) axis under `direction:rtl` runs right→left (Broiler laid RTL out identically to LTR) — mirror the resolved column edges within the content box for in-flow items (also flips `justify-content` start↔end) and around the padding box for abspos items (§9.2); the D test's 8 `directionRTL` variants now resolve exactly (guard `GridRtlTests`, 0 regressions). Remaining: A's vertical fit-content + baseline, vertical-rtl (E test 16), the harness −1 offsetParent-border estimator nuance, and full CI pixel scores; Workstreams F/G still open. | 🟡 partial | Broiler.Layout/CssBoxGrid |
| 38 | **`css/css-text` first CI run (#1274, 1083 fails, all in one directory).** Triaging the `PixelMismatch/ReferenceOverlayExposed` Ahem "green-square/no-red" sub-cluster (`line-break/line-break-anywhere-overrides-uax-behavior-*`) exposed **three** independent, compounding root causes — the report's category was, as usual, misleading (it read as one MissingContent bug). All three had to land together for the family to render the expected square. **(1) `line-break: anywhere` unimplemented (main repo).** The property was never stored or read, so `XX﻿XX` in a `width:2ch` box laid out as one unbreakable run overflowing on a single line (the reference red fully exposed). Added the inherited `LineBreak` property (`CssBoxProperties`/`CssUtils` get+set) and routed `line-break:anywhere` through the same per-character word split as `word-break:break-all` in `CssBox.ParseToWords` (a soft-wrap opportunity between every typographic unit, §5.1/§5.4). The split had to be restructured to **decode HTML entities as a whole run before splitting** — the raw box text holds undecoded entities (`&#xFEFF;`), so a naïve per-character split of the raw text emitted `&`,`#`,`x`,… instead of one zero-width code point; surrogate pairs stay intact. Guard `LineBreakWordSplittingTests`. **(2) `font` shorthand dropped the family on a spaced slash (Broiler.CSS submodule).** `font: 50px / 1 Ahem` (white space around the `size/line-height` slash) tokenized the `/` as its own token; the size classifier read `50px` as a size with no line-height and folded `/ 1 Ahem` into font-family — an unmatchable family, so Ahem never loaded and every glyph fell back to a proportional font at ½ the advance. Fixed by gluing the slash back onto the size token (`NormalizeFontSlashTokens`) before classification. **Push 403 at authoring → `patches/0013-css-font-shorthand-slash-whitespace.patch`; now applied upstream** — `Broiler.CSS` `88adbf0` is an ancestor of the pinned `2aad88b`, so the spaced-slash fix is live on CI (the patch file is retained for provenance, pointer already pins a descendant); no parent-repo fallback exists (font expansion is entirely engine-side). Guard `CssStyleEngineTests.Font_Shorthand_Expands_With_Whitespace_Around_LineHeight_Slash`. **(3) `ch` unit hard-coded to 0.5em (main repo).** `CssLengthParser` approximates `1ch` as `em/2`; correct-ish for proportional fonts but wrong for Ahem, whose "0" is a full `1em` — so `width:2ch` came out 50px not 100px and only one 50px glyph fit per line. Resolved `ch` from the real "0" advance by measuring it against the box's font (`CssBoxProperties.GetChWidth` overridden in `CssBox` via `LayoutEnvironment.MeasureText`, intercepted in `ParseLengthWithLineHeight` alongside the existing `rem` special-case), falling back to the parser approximation when no measuring environment is wired up. With all three, `line-break-anywhere-{002..006}` go **98%→pass (99.5%)** (001's residual is a separate U+2060 glyph-advance detail); **0 regressions** on the committed css-align/css-anchor-position/css-backgrounds/CSS2 WPT subsets (102/102 pass unchanged) and the layout/CSS unit suites. The dominant `#1274` clusters — hyphenation (965, needs a real hyphenation dictionary), CJK `i18n` line-break + `seg-break-transformation` (need a CJK font + the §4.1.2 segment-break-removal rule + `ch` metrics) — remain feature-depth tails. | 🟡 partial | Broiler.Layout (+ Broiler.CSS patch 0013) |
| 39 | **`css/css-text` second CI run (#1276, 1034 fails, still all in `css-text`).** The report's "top dropped CSS declarations" flagged `text-align: justify-all` (30) and, adjacent to it, the `text-align/text-align-last-{N}.html` family (14) — Broiler had **no `text-align-last` property at all** and dropped the `justify-all` shorthand value as invalid. `text-align-last` (CSS Text 4 §text-align-last: `auto | start | end | left | right | center | justify | match-parent`) governs the alignment of a block's **last** line, overriding `text-align` there; `text-align: justify-all` additionally justifies that last line (a plain `justify` leaves it ragged). Implemented `text-align-last` end-to-end in `Broiler.Layout`: the inherited `CssBoxProperties.TextAlignLast` property + `CssUtils` get/set plumbing, and a rewrite of `CssLayoutEngine.ApplyHorizontalAlignment` to resolve the effective per-line keyword — the last line routes through the new `ResolveTextAlignLast` (explicit `text-align-last` wins; `auto` follows `text-align`, except a justified block's last line stays `start`-aligned unless the value was `justify-all`). `ApplyJustifyAlignment` no longer self-skips the last line (the caller decides) and a single-word line now stays flush at the start instead of being pushed to the right edge (a latent quirk that also mis-placed overflowing single words). The `Broiler.CSS` validator gains `text-align:justify-all` acceptance + explicit `text-align-last` validation + inherited-set membership. **`text-align-last` is live on CI now** — it survives the pinned validator's default-accept path, so the layout applies it immediately (guard `TextAlignLastTests`: right/center/override + `text-align-last:justify` last-line stretch). **`text-align:justify-all` validator acceptance → `patches/0014-css-text-align-last-justify-all.patch` (push 403 at authoring; now applied upstream** — `Broiler.CSS` `953cc07` is an ancestor of the pinned `2aad88b`, so `justify-all` is accepted on CI and the layout applies it; the patch file is retained for provenance). The shared last-line-justify path is also exercised via `text-align-last:justify`. 0 regressions on the vendored css-align/css-anchor-position/css-backgrounds/CSS2 subsets + the layout/CSS unit suites. The dominant `#1276` tails — hyphenation (939, needs a hyphenation dictionary), CJK `i18n` line-break + `seg-break-transformation` (939/113/91, need a CJK font + §4.1.2 metrics), and `display:-webkit-box` (45, needs the legacy flexbox) — remain feature-depth work. | 🟡 partial | Broiler.Layout (+ Broiler.CSS patch 0014) |
| 40 | **`css/css-writing-modes` "biggest problems" (#1279, `block-flow-direction-vlr-020` / `-slr-060` at ~13%).** Both render "PASS" as a grid of Ahem cells inside a `display:table-caption; height:9em` box in a vertical writing mode. Three stacked layout bugs, each spec-grounded: **(1) table-caption not a containing block.** `CssBox.ContainingBlock` whitelisted block/inline-block/list-item/table/table-cell but **not `table-caption`** (a block container that establishes its own BFC, CSS2.1 §17.4), so the caption's in-flow children resolved their width against the viewport (1008px) instead of the caption's inline size; in the vertical-flow prototype that viewport width became the caption's **physical height** after the rotation, so the blue box filled the whole viewport. **(2) physical border/padding read on the wrong logical axis.** A transposed vertical box laid out in the logical (horizontal) frame read its authored `border-top`/`bottom` (and padding) as the frame's block-axis insets, so they inflated the box's post-rotation **width** (block axis) rather than its height (inline axis) — the blue box came out ~2.3× too wide (960 vs 420). Fixed by resolving a transposed box's border/padding through the logical→frame edge map **only during the frame-layout phase** (`CssBoxProperties.UsesLogicalFrameInsets` gated on a ThreadStatic `InVerticalFrameLayout` set around the rotation root's `PerformLayoutImp`); the later paint pass reads the authored physical borders unchanged, so a box's own borders never visually rotate. **(3) sideways-lr inline direction + glyph facing.** `sideways-lr`'s inline axis runs bottom→top (not top→bottom like the other vertical modes) and its glyphs face 90° **counter-clockwise**; without both, the caption rendered "PASS" vertically mirrored. Flipped the inline map about the box's inline extent in `TransformVerticalSubtree` and set the Stage-2 glyph rotation to −90° for `sideways-lr` in `FragmentTreeBuilder`. With all three the blue box is **pixel-exact** and the full test matches a locally-generated Chromium reference at **99.5%** (vlr) / passes (slr) — verified in `--render` mode where Ahem loads; **0 regressions** against freshly-generated Chromium references for the vendored css-align (18/28) and css-anchor-position (32/40) subsets. Guard `VerticalTableCaptionTests` (both modes collapse to a bounded ~180px-tall region, not the viewport). **Runner-scoring caveat (separate, pre-existing):** the scored `RunSingleTest` path resolves the page's fonts during its DOM pass *before* `EnsureWptFontsLoaded` registers Ahem, so the visible Ahem glyphs paint with a fallback face and cap these two at ~96% in the runner even though the geometry is now correct (the layout is verified correct via `--render`, which loads fonts first). Reordering the load to fix it made Ahem render for **every** test and regressed the vendored css-align set 18→4 against fresh Chromium refs (Broiler's Ahem edge rasterisation diverges sub-pixel from Chromium's on the bar-based tests, where the fallback happened to match closer), so that reorder is deliberately **not** shipped here — it is a font-fidelity/harness concern orthogonal to the layout fix. | 🟡 partial | Broiler.Layout |
| 41 | **`css/css-backgrounds` "biggest problems" (#1284, `background-margin-root` / `-transformed-root` / `-will-change-root` at ~6%).** A root `<html>` with `background: linear-gradient(lightblue, yellow); height:300px; margin:50px` must propagate its gradient to the **canvas** as a 300px-tall tile phased at the margin edge and **repeated** over the whole viewport (CSS Backgrounds §14.2 painting area = canvas, positioning area = root box). Broiler painted a single gradient stretched over the entire viewport. Two independent, spec-grounded root causes: **(1) canvas gradient tile sized from the paint area, not the positioning area.** `PaintWalker.EmitGradientLayers` defaulted an `auto` tile to `fillRect` (the viewport for a propagated root background) instead of the source element's background positioning area (§3.9), so one tile filled the canvas with no repeat. Fixed by computing the positioning area first and sizing the tile from it; ordinary element backgrounds are unaffected (there positioning area ≡ paint area). **(2) gradient line length used `max(W, H)`.** `BCanvas.GetGradientEndpoints` (the `Broiler.HTML.Image` raster canvas) took the half-length as `max(W,H)/2`, which over-extends the gradient line on a non-square tile (924×300 here) and compresses the visible colour run to the middle band (top row came out pale-green, not lightblue). Replaced with the CSS Images 3 §3.4.2 length `abs(W·sin A) + abs(H·cos A)` (square tiles unchanged, so the committed refs and the gradient unit test are byte-identical). With both, all three tests match freshly-generated Chromium references **pixel-for-pixel (6 %→100 %)**; **0 regressions** across the vendored css-backgrounds (39/61), css-align (19/28), css-anchor-position (29/35), CSS2 (13/17) subsets (byte-identical failure sets) and the graphics parity suite. **Push 403 at authoring → `patches/0015-css-backgrounds-root-gradient-propagation.patch`; now applied upstream** — `Broiler.HTML` `058af9e` is an ancestor of the pinned `5c16c12`, so both fixes are live on CI (the patch file is retained for provenance); they live entirely in the `Broiler.HTML` paint layer with no main-repo path to mirror. The issue's other listed items are out of scope for this fix: `background-image-shared-stylesheet` (0 %) needs dynamic iframe navigation + WPT server `?pipe=trickle` and is untestable in a static renderer; `clip-border-area-on-body-not-propagated-to-root` (reported 8.9 %) already renders correctly — `background-clip:border-area` is handled and both `--render` and the scored runner give ~99 % (the residual is single-line text anti-aliasing); the `*-interpolation` animation tests are `interpolation-testcommon.js` harness-driven. | 🟡 partial | Broiler.HTML (patch 0015) |
| 43 | **`css/css-conditional` "biggest problems" (#1305, five `css-supports-*` reftests at 0 % match).** `css-supports-005/009/010/020/030` all assert that an `@supports` rule whose condition is *valid syntax but a failing condition* must **not** apply its inner `html { background-color: red }`, leaving the page green (`support/pass.xht`). Broiler rendered red on all five. Root cause: `@supports` gated only on **grammatical validity** and evaluated the *truth* of every well-formed feature query **optimistically** (assumed supported), so `(color: rainbow)`, `(color: rainbow) and (color: blue)`, `(unknown: green)`, and the `<general-enclosed>` `(not (color: rainbow) or (color: green))` all counted as true and applied the red rule. Fixed by evaluating the condition for real: `SupportsConditionSyntax` now returns a tri-state (`Invalid`/`False`/`True`) — a feature query `( <decl> )` resolves through a support oracle, a `<general-enclosed>` group evaluates to **false** (making `not (@page)` correctly *true*), and `and`/`or`/`not` fold the results per the css-conditional grammar; the cascade applies the rule only when the result is `True`. The oracle (`CssStyleEngine.IsFeatureQuerySupported`) accepts a query only when the property is a **recognised CSS property** and the value is **valid for it** — a full CSS named-color/system-color/color-function `<color>` check rejects `rainbow`/`iridescent`, and an unknown property (`unknown`) is rejected; optimism is preserved for recognised properties Broiler may not fully render (their queries stay true, matching the Chromium-generated references). Verified end-to-end with `--render`: 005/020/030 now render **byte-identical to an all-green baseline** (were all-red), a supported query and `not (@page)` still apply, and the full css-supports family truth table is a regression guard (`CssStyleEngineTests.Supports_Rule_Applies_Only_When_Condition_Evaluates_True`, 30 cases). CSS.Dom (140/141; the one failure is the pre-existing path-string `CssDomArchitectureTests`) and CSS kernel (63/63) suites otherwise green. **Push 403 at authoring → `patches/0016-css-supports-feature-query-evaluation.patch`; now applied upstream** — `Broiler.CSS` `7de38a7` (on `0d0ef69`) is an ancestor of the pinned `2aad88b`, so the five tests are gated correctly on CI (the patch file is retained for provenance); the `@supports` gate is entirely engine-side (`CssStyleEngine.CollectFromRules`) with no main-repo interception point (as with patches `0013`/`0014`). The issue's other listed items are separate clusters: `background-image-shared-stylesheet` (0 %) needs iframe navigation + `?pipe=trickle` (out of scope, per cluster 41); the three `css-color-adjust/dark-color-scheme` items need `color-scheme` canvas propagation and the `container-queries/top-layer-dialog-backdrop` item needs `::backdrop`/top-layer — each its own feature-depth increment. | 🟡 partial | Broiler.CSS (patch 0016) |
| 42 | **`css/css-backgrounds` "biggest problems" (#1288, top item `clip-border-area-on-body-not-propagated-to-root` reported at 8.9 %).** Cluster 41 recorded this test as "renders correctly, scored runner ~99 %" — that conflated the `rel=match` reference-HTML comparison with the **scored** comparison. The scored path compares Broiler's render against the *committed golden PNG*, which is a Chromium screenshot of the **test file** — and here that golden is **wrong**: `background-clip: border-area` is unsupported by the reference-generating Chromium, so it fills the whole `<body>` box solid green (an all-green 1024×768 PNG). Broiler renders the spec-correct 20px green border ring (byte-identical to its own render of the `-ref.html`, and to the committed `…-ref.png`), so against the wrong all-green golden it scores 8.9 %. In other words **Chromium fails its own reftest** — its test screenshot disagrees with its reference screenshot — which makes the test golden invalid. **Runner fix (Broiler.Wpt, no submodule): reftest golden fallback.** `WptTestRunner.RunTest` now, on a pixel-mismatch against the committed test golden, resolves the test's `rel=match` reference file's committed golden (`TryResolveReferenceGoldenPath`, mirroring the test's own golden-path derivation) and, if Broiler's render matches *that*, passes the reftest. It is reached only after the primary comparison already failed, so it can only add passes (never mask a Broiler bug: a wrong render matches neither golden) and is regression-free by construction (baseline vs. fixed on the vendored css-backgrounds subset both 39/61). Guards `RunTest_Passes_Reftest_Against_Reference_Golden_When_Test_Golden_Is_Wrong` + `_Does_Not_Pass_When_Broiler_Matches_Neither_Golden`. This generalises the pre-existing #14 `SuspectReference` diagnostic from an annotation into a scoring decision for the "Chromium fails its own reftest" class (aligns with the same principle behind cluster 32). **This specific test still does not flip to a full pass**, because a *second, orthogonal* gap keeps even the correct comparison at 99.0 % (just over the ≤1 % threshold): the compat/headless text backend does not apply registered font faces to the render (verified: explicit `font-family: 'Ahem'`/`serif` render in the bundled fallback, not the requested face), so the single line of body text lays out ~11 % wider than Chromium's Liberation Serif (rightmost ink 879px vs 794px) — a cross-cutting font-fidelity limitation, not a css-backgrounds bug, deferred as its own workstream. The issue's other four items are out of scope for the same reasons as cluster 41 (`shared-stylesheet` needs iframe navigation + `?pipe=trickle`; the three `*-interpolation` tests are `interpolation-testcommon.js` / Web-Animations harness-driven). | 🟡 partial | Broiler.Wpt |
| 44 | **`css/css-values/calc-in-{calc,max}` (#1316, 0 %).** A green abspos box (`height: calc(calc(100%))` / `max(calc(100%))`, `width:100%`) must cover a red `html` background under `html{overflow:hidden}`. Nested calc parsing was never the bug (`calc(calc(200px))` already resolves). The abspos percentage-height half is now handled on `main` (`HeightPercentageResolvesToAuto` abspos exemption + `PercentageHeightContainingBlockHeight`); the remaining gap was **root `overflow` clipping at the html box, not the viewport**. CSS Overflow 3 §3.3 / CSS2.1 §11.1.1: a clipping `overflow` on the root element is applied to the viewport and the element's own used overflow becomes `visible`. For a short document (empty body + abspos children) the html box is ~0px tall, so out-of-flow descendants were clipped away (all-red). Fixed by taking the root `<html>` element's used overflow as `visible` when building its `ComputedStyle` (`ComputedStyleBuilder.RootOverflowUsedValue`); the render surface (== viewport) provides the clip. With both, `calc-in-calc` and `calc-in-max` render fully green. Main-repo (`Broiler.Layout`). Guard: `RootOverflowAbsposPercentHeightTests`. | ✅ fixed | Broiler.Layout |
| 45 | **`css/css-inline/empty-span-scroll` (#1316, 0 %).** Sibling `<span>`s each wrap a block `<div>` — a 3000px red filler, an abspos empty `<a>` target inside nested spans, then a green `100vh` box; `target.scrollIntoView()` must show the green box. Broiler rendered all-red because `scrollIntoView` computed a scroll offset of 0. Root cause in `DomBridge.ComputeOffsetWithinParent`: it summed each preceding sibling's CSS `height`, but an inline `<span>` wrapping a block (CSS2.1 §9.2.1.1 block-in-inline) has `height:auto` → 0, so the filler contributed nothing. Fixed by falling back to the sibling's laid-out border-box extent for inline boxes that contain block-level content (`IsInlineBoxWithBlockContent`), gated to the block axis; percentage sibling sizes now resolve against their containing block (gated to `%` values to keep the walk O(n)). Main-repo (`Broiler.HtmlBridge.Dom`). Guard: `BlockInInlineScrollIntoViewTests`. | ✅ fixed | Broiler.HtmlBridge.Dom |
| 46 | **`css/css-content/element-replacement-root-canvas-bg-from-body` (#1316, 0 %).** `:root { content: url(rect.svg) }` replaces the root; its descendants generate no boxes, so the `<body>` background must NOT propagate to the canvas (CSS Content 3 + CSS Backgrounds 3 §body-background). Broiler rendered aquamarine (body bg) across the canvas plus body text. Fixed with a render-prep step (`DomBridge.ReplaceRootWithReplacedContent`), gated to a root whose computed `content` is a `url()`: it strips the root/body backgrounds and replaces the body with a single `<img>` at the origin — byte-identical to the reference (green 100×100 square, white canvas). Main-repo (`Broiler.HtmlBridge.Dom`). Guard: `RootContentReplacementTests`. | ✅ fixed | Broiler.HtmlBridge.Dom |
| 47 | **`css/css-position/sticky/position-sticky-fixed-ancestor-002/003` (#1316, 0 %).** First increment of `position: sticky`, which Broiler never offset (treated as static). A green `bottom:0` viewport-tall sticky box inside a viewport-sized **fixed** container after a viewport-tall spacer must pin to the scrollport's bottom edge and fill the viewport. Implemented as a bridge render-prep pass (`DomBridge.ResolveStickyPositioning`, before `ApplyScrollSimulation`): for each sticky box, compute the physical top/bottom/left/right inset constraints against its nearest scroll container (or viewport) at the current scroll offset, clamp to the containing block, and rewrite to `position:relative` with that offset. Both tests render byte-identical to the shared reference (full-viewport green "PASS"). **Scope:** pins at the current scroll offset; dynamic scroll+sticky and vertical writing modes are follow-up increments. Main-repo (`Broiler.HtmlBridge.Dom`). Guard: `StickyPositioningTests`. | 🟡 partial | Broiler.HtmlBridge.Dom |
| 51 | **Nested abspos with auto insets flew to the containing-block top (y=0) instead of its static position.** Root-caused while taking on `css-align/self-alignment/block-justify-self` (the worst css-align failure): an absolutely-positioned box with auto insets whose **preceding sibling is inline/text** (e.g. `<div>text<div style="position:absolute"></div></div>`) lost its parent's content-top offset — the static-top formula used base `0 + flowPrevBottom`, and an inline sibling records `ActualBottom = 0`, so the box resolved to the containing block's top (y≈0) rather than its parent's content area. A block preceding sibling (absolute `ActualBottom`) and a first child were already correct. Fixed by basing the static top on `ParentBox.ClientTop` with the in-flow advance clamped to ≥0 (`Math.Max(0, flowPrevBottom - baseTop)`) — **byte-identical for block-after-block and first-child**, only correcting the inline-preceding-sibling case. General correctness win (nested abspos is common). Improves `block-justify-self` (86.0%→86.6%, the pure-red overlay shrinks) but does not flip it — the box still lands ~20px above Chromium's placement (the anonymous line box after the leading inline text is not yet modelled), and the test's `justify-self` edge cases (RTL physical `left`/`right`, abspos justify-self) remain. Main-repo (`Broiler.Layout`). 0 regressions (css-align 19 / css-anchor-position 29 / css-backgrounds 39 / CSS2 13 unchanged; 262 Layout unit tests green). Guard: `NestedAbsposStaticPositionRenderTests`. | 🟡 partial | Broiler.Layout |
| 50 | **`background-clip:text` ignored on `<table>` elements.** In `css-backgrounds/background-clip/clip-text-descendants` (`background-clip:text` over 8 descendant types), Broiler rendered the block/float/inline-block rows correctly purple (blue background clipped to semi-transparent-red text) but the two `<table>` rows salmon — the clip was not applied. Root cause: `PaintWalker.PaintChildren` routes `table`/`inline-table` boxes to `PaintTableChildren`, which recursed via `PaintFragment` **without** the `bgClipTextColor` the background-clip:text machinery propagates to descendants, so table cell text was never composited with the ancestor background. Fixed by threading `bgClipTextColor` through `PaintTableChildren` — all 8 rows now render purple (verified via `--render`). `Broiler.HTML` submodule (push 403 → `patches/0012-…`, pointer unbumped). The reftest itself stays pixel-gated on orthogonal table-layout metrics (row heights/float placement shift the lines) + glyph AA, so it does not flip, but the specific clip bug is fixed. 0 regressions (css-backgrounds 39/61 unchanged). | 🟡 partial | Broiler.HTML (patch 0012) |
| 49 | **List-item markers built but never painted (all `display:list-item` elements — real `<ul>`/`<ol>` lists included).** `CssBox.CreateListItemBox` (`Broiler.Layout`) lays out a generated marker box (bullet/number) for every list item, but the marker is not a member of `box.Boxes`, so the layout→fragment builder never emitted it and **no list rendered a marker at all** (verified with a minimal `<ul><li>` / `display:list-item` div / `list-style-type:decimal` repro — all rendered with no marker). Fixed in the main repo (`FragmentTreeBuilder.Build` lives in `Broiler.Layout/IR`, not the `Broiler.HTML` submodule): exposed the marker via `CssBox.ListItemMarkerBox` and projected its glyph word into a dedicated line fragment (`BuildListMarkerLineFragment`), suppressed under `content-visibility:hidden` and `list-style-type:none`. Markers now paint (`•`/`o`/`♠`/decimal). 0 test-count regressions on the vendored css-align (19)/css-backgrounds (39)/CSS2 (13)/css-anchor-position (29) subsets; all 262 `Broiler.Layout` unit tests green. Guard: `ListItemMarkerRenderTests`. | ✅ fixed | Broiler.Layout |
| 48 | **`css/css-anchor-position/anchor-size-css-zoom` (69.2 %, worst vendored css-anchor-position failure).** `anchor-size(--a width/height)` under a `zoom: 2` containing block was **zoom-counted twice**: the anchor's registered geometry comes from its real laid-out box (`TryGetAnchorLayoutBox`, shared geometry), which is already in used/zoomed pixels (a `200×100` anchor measures `400×200`), and `ResolveAnchorSizeFunctions` wrote that used size straight onto the positioned element's `width`/`height` — which the DOM `zoom` bake then scaled by 2 again, giving an `800×400` green box instead of `400×200`, so the green target overhung the red anchor (the test's "no red visible" invariant failed). Fixed by dividing the resolved `anchor-size()` length by the positioned element's own used zoom (`GetUsedZoomForElement`), landing it in the element's pre-zoom CSS-px frame before the bake re-applies zoom — matching the reference (a zoom:2 anchor's 200px width covers a zoom:2 positioned box). A no-op for the common unzoomed case (used zoom == 1), so the other css-anchor-position anchor-size tests are byte-identical. Test now renders **identical to the reference** and the runner passes it (69.2 %→99.7 %; css-anchor-position regular 28→29 pass, average match 97.98 %→98.76 %, 0 regressions on the vendored css-anchor-position/css-align subsets). Main-repo (`Broiler.HtmlBridge.Dom`). Guard: `NativeAnchorSizeZoomWptTests`. | ✅ fixed | Broiler.HtmlBridge.Dom |
...
- **List-item markers were built but never painted — ✅ fixed (cluster 49).** `CssBox.CreateListItemBox`
  (`Broiler.Layout`) builds and positions a `_listItemBox` (glyph/number, `Left = Location.X - width - 5`)
  for every `display:list-item` element, but **no paint path emitted it** — there was no `_listItemBox`
  reference in the fragment/`PaintWalker` pipeline, and a minimal repro confirmed a real `<ul><li>`, a
  `display:list-item` `<div>`, and `list-style-type:decimal` all rendered **with no marker**. The
  layout→fragment builder (`FragmentTreeBuilder.Build`, **main repo** — it lives in `Broiler.Layout/IR`,
  not the `Broiler.HTML` submodule) never emitted the detached marker box. Fixed by exposing the marker
  (`CssBox.ListItemMarkerBox`) and projecting its glyph word into a dedicated line fragment
  (`BuildListMarkerLineFragment`), gated off `content-visibility:hidden` and `list-style-type:none`. See
  cluster 49.
