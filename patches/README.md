...
**Pending because the push was denied.** `Broiler-Platform/Broiler.HTML` is
outside this session's GitHub scope, so the git proxy declined to inject a
credential and the push returned 403. The submodule pointer is therefore **not**
bumped — CI clones submodules by pointer and would break on a commit it cannot
fetch.
