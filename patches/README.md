...
checked-out tree (reverse-apply succeeds) is skipped, so 0013–0016 (in the pinned pointer) are
never re-applied, and 0012 stops being applied automatically the moment a maintainer lands it
upstream and the pointer is bumped. The build compiles submodule source in place, so patching the
working tree is sufficient — no commit, no pointer bump; only Broiler's render is affected, not
Chromium reference generation.
