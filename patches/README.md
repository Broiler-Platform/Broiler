...
**`0102` is pending against the `Broiler.JS` pinned pointer `14fa4f10`**, on the usual terms: the
...
pin plus that patch. It is the only pending file and depends on nothing else, so there is no
ordering constraint to preserve. It needs **no main-repo fallback**: the counter is off by default
and changes no emission on any setting, so nothing CI runs behaves differently with or without it.
