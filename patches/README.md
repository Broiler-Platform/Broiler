...
| --- | --- | --- |
| `0103-js-widen-census-corpus` | `Broiler.JS` | **The corpus every phase-3 and phase-4 headline is computed over was 7 of Octane's 15 suites, and the reason had never been written down.** Item 2-13 established that this engine's largest *private* deficiency is **zlib — 12.0× behind Jint relative to Chromium** — so the next step was to ask what the censuses say about it. They say nothing: zlib is in none of them, and neither are Mandreel, Gameboy, PdfJS, Typescript, CodeLoad, Splay or RegExp. `TypeFeedbackMetrics`, `SpecializingTierMetrics` and `PropertyMapDistributionMetrics` all ran the same seven, whose output the roadmap quotes as ***"the corpus"*** over a denominator it never states. **Why seven is not a choice about cost**: widening the list aborts at the ninth suite with an **uncatchable .NET stack overflow** in Mandreel's `global_init`. Item 0-2's 16 MiB thread and 4 MiB reserve are a property of **the shell** (`Program.cs`), and `BenchmarkContext.Create` built a plain `JSContext` on whatever stack the runtime handed `Main` — *every benchmark host in this campaign has been running without the reserve the shell has had since phase 0*, so the one suite that needs it was permanently unmeasurable. **And the instrument made finding out as expensive as possible**: it serialized once, at the end, so the abort discarded the eight suites that had already run. Both fixed — the census runs on the shell's thread with the shell's `MaxStackUsageBytes` and writes after **every** suite, and Mandreel now fails catchably instead of taking the process down. The corpus aggregate is **emitted by the instrument with its suite count beside it** rather than left to be totalled by whoever quotes it, which is the step where the suite list stopped travelling with the number. **The widened count moves phase 4's premise.** The old seven reproduce **to the digit** — 93.54% of 37.9 M reads, 96.70% of 4.24 M calls, §0's phase-4 row verbatim — so this is the same instrument over a bigger corpus and not a different measurement. Every suite that runs: **80.11% of 307.9 M reads and 86.35% of 52.9 M calls**, with **87.7% of the reads outside the seven**. The two suites that move it had never been counted — **Gameboy at 34.60% monomorphic with 1 282 polymorphic sites, and Splay at 10.15%**, the two most polymorphic suites in Octane. 80% is still most of the work, so 4-2b is not retired; what is retired is that 93.5% described the engine rather than seven suites picked for being call-heavy. **Honest limits, stated in the output**: zlib (`read` builtin lives in the shell), Mandreel (now hits the guard) and RegExp (pre-existing checksum) report setup-only counts — together **0.09% of reads** — so the headline is over **12** suites and the JSON says `suiteCount: 12`. **Purely additive to behaviour**: two benchmark-host files, no engine source touched, and nothing that ships changes. **Applies to `14fa4f10`; independent of `0102`, though both were verified applying in sequence with `git am --keep-cr`.** |
...
**`0102` and `0103` are pending against the `Broiler.JS` pinned pointer `14fa4f10`**, on the usual terms: the
...
pin plus those patches. They are **independent of each other** — `0102` touches the compiler and
its diagnostics, `0103` two benchmark-host files — so there is no ordering constraint, though both
were verified applying in sequence to a clean checkout of the pin with `git am --keep-cr`. Neither
needs a **main-repo fallback**: `0102`'s counter is off by default (`BROILER_JS_DEFER_TREE_COUNT=1`)
and changes no emission on any setting, and `0103` touches only the benchmark host, so nothing CI
runs behaves differently with or without either.
