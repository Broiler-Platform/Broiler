...
**`0067`–`0071` are pending against the pinned pointer `9bf9639b`.** All five were applied in
...
follow `0067`**: it edits the same file and builds on the gate `0067` restructures. `0069`,
`0070` and `0071` touch different assemblies again and apply in any order relative to the
others.
...
CI is correct without it and one refactor less safe; and `0071` adds a facility nothing calls
yet, so without it CI is identical.
