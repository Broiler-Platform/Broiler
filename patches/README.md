...
| 0024 — `CssStyleEngine`: expand a CSS-wide-keyword `background` shorthand to its longhands | `Broiler.CSS` | **PENDING upstream** (push 403; pinned `5bcefb3` does not contain it) — **applied on the WPT CI run** by `scripts/apply-pending-wpt-patches.sh`. Generated against the current pinned `5bcefb3`; `git apply` passes. | **WPT issue [#1451](https://github.com/Broiler-Platform/Broiler/issues/1451) — css-view-transitions-2 nested-group tests `normal-goes-up` / `contain-on-self`.** `background: inherit` (and `initial`/`unset`/`revert`) is a CSS-wide keyword for the *whole* shorthand and must set every `background` longhand to that keyword (CSS Cascade §7.3); `ExpandBackgroundShorthand`'s per-layer parser skipped the keyword and left `background-color` at its initial `transparent`, so `background: inherit` never inherited the parent's colour. Detect a sole CSS-wide keyword up front and set each longhand to it. **No main-repo fallback** — the shorthand expander is entirely engine-side. **Main-repo companions (on the parent branch, no patch needed):** (a) `CssBoxProperties.BackgroundColor` resolves `inherit` against the parent's already-cascaded colour so a *chain* of `background-color: inherit` boxes each inherits the resolved colour rather than the literal `inherit` (which `GetActualColor` folded to opaque black) — this is what the group-children wrappers inherit through; (b) `CssBox.ContainingBlock` sizes nested inset-sized absolutes (commit *Size abspos boxes…*). Both are general fixes, inert-to-helpful without this patch. **Verified with `Broiler.Wpt --render`, patch applied:** `normal-goes-up` and `contain-on-self` render all-green (matching `nested-ref.html`), with the earlier `nearest-direct`/`compute-explicit-name-direct`/`nested-vt-names` still green. Guard: `ViewTransitionRenderTests` nested cases feature-probe and self-skip on the un-patched engine; `BackgroundColorInheritChainTests` guards the main-repo chain fix unconditionally. |
