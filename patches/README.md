...
**`0072` and `0073` are pending against the pinned pointer `72494502`.** Both were applied in
sequence from a clean checkout of that commit with **`git am --keep-cr`**, and the resulting tree
was diffed against the branch they were generated from — identical. **`0073` must follow `0072`**:
they edit the same files, and `0073` builds on the site range `0072`'s gate change makes room for.
...
needed:** both live entirely inside the JavaScript engine's tiering path, which is **off by
default** (`FunctionTieringOptions.Enabled` is `false`, and `SpecializeFromTypeFeedback` too), so
CI is unaffected either way. What CI does *not* get without `0072` is the DeltaBlue fix — but that
defect is only reachable by a host that opts into tiering, which nothing in this repository does.
