...
# The findings this run would file as GitHub issues, and how much of each one a
# body carries before it stops being readable.
ISSUE_DIR_NAME = "issues"
ISSUE_INDEX_NAME = "index.json"
ISSUE_KIND_REGRESSIONS = "regressions"
ISSUE_KIND_UNMEASURED = "unmeasured"
ISSUE_KIND_GAPS = "gaps"
DEFAULT_GAP_ISSUE_LIMIT = 10
DEFAULT_REGRESSION_ISSUE_LIMIT = 20
ERROR_SIGNATURE_CHARS = 160
# Incidental detail that makes two reports of the same missing API look like two
# different errors: the value that was passed, a URL, an index, a timestamp.
...
# ── GitHub issues ───────────────────────────────────────────────────────────
#
# The WPT runners file what a run found as GitHub issues rather than leaving it
# in an artifact that expires in 30 days, and this suite's findings have the
# same shape: a backlog nobody is going to download a zip to read.  Three
# documents are built rather than one, because ranking them together would bury
# the two small ones under the large one — the gap list is hundreds of probes
# wide and permanent, while a regression is a handful of probes and new:
#
#   regressions — probes that used to produce a value and no longer do, plus
#                 pages that stopped running at all.  The only one of the three
#                 that describes a change this repository made, so it is filed
#                 first and is what fails the run.
#   unmeasured  — pages neither engine measured: Broiler could not run them, or
#                 no Chromium reference was captured to compare against.  Filed
#                 because an empty gap list on such a page means nothing was
#                 looked at, not that there was nothing to find.
#   gaps        — probes Chromium answers and Broiler does not, grouped so that
#                 one fix reads as one row: thrown-error signatures first (one
#                 signature is usually one missing API gating many probes), then
#                 the pages carrying the most gaps.
#
# Each body carries a hidden marker naming its kind.  The suite runs weekly and
# its backlog changes slowly, so a run that files the same finding again would
# bury the one a reader is already working from; the marker is how the workflow
# finds the issue already open for a kind and refreshes it in place instead.


def issue_marker(kind: str) -> str:
    """The hidden body marker that identifies one kind of issue across runs."""
...
def error_signature(value: Any, limit: int = ERROR_SIGNATURE_CHARS) -> str:
    ...
    The pages store what a probe threw where its result belongs, so the message
    arrives as a JSON-encoded value.  Two pages reporting the same missing API
...
def _issue_footer(run_url: str | None) -> list[str]:
    return [
        "",
        "### CI metadata",
        "",
        f"- Workflow run: {run_url}" if run_url else "- Workflow run: (unknown)",
        "- Artifact: `privacy-test-pages-results` — `gaps.md` lists every probe with what "
        "Chromium returned beside it, `report.md` and `report.html` the per-page detail.",
        "",
        "_Auto-generated by `.github/workflows/privacy-test-pages.yml`. Later runs of the "
        "suite refresh this issue in place rather than filing it again._",
    ]


def _issue_provenance(document: Mapping[str, Any]) -> str:
    reference = document.get("reference", {})
    environment = document.get("environment", {})
    return (
        f"Chromium `{reference.get('chromiumVersion') or 'n/a'}` via Playwright "
        f"`{reference.get('playwrightVersion') or 'n/a'}` · Broiler commit "
        f"`{environment.get('gitCommit') or 'n/a'}` · run {document.get('generatedAt') or 'n/a'}."
    )


def _probe_list(probes: Sequence[str], limit: int = 8) -> str:
    shown = ", ".join(f"`{probe}`" for probe in probes[:limit])
    return shown + (f", … {len(probes) - limit} more" if len(probes) > limit else "")


def build_gap_issue(
    document: Mapping[str, Any], *, limit: int = DEFAULT_GAP_ISSUE_LIMIT
) -> dict[str, Any]:
    """Rank the gap document so that one fix reads as one row.

    The gap document is per page and per probe, which is the right shape for
    evidence and the wrong one for a backlog: a missing API shows up once per
    probe that touches it.  Grouping the thrown errors by signature is what
    turns those repetitions back into the single finding they are.
    """

    signatures: dict[str, dict[str, Any]] = {}
    pages: list[dict[str, Any]] = []
    for page in document.get("pages", []):
        gaps = page.get("gaps") or []
        if not gaps:
            continue
        pages.append({
            "id": page["id"],
            ...
            "url": page["url"],
            "gaps": len(gaps),
            "errors": sum(1 for gap in gaps if gap.get("broiler") == "error"),
            "probes": [gap["test"] for gap in gaps],
        })
        for gap in gaps:
            ...
            if len(entry["examples"]) < 3:
                # Qualified by page: the same probe name on two pages is two
                # findings, and an unqualified list reads as one repeated.
                entry["examples"].append(f"{page['id']}:{gap['test']}")

    ranked_signatures = sorted(signatures.values(), key=lambda entry: (-entry["count"], entry["signature"]))
    ranked_pages = sorted(pages, key=lambda page: (-page["gaps"], page["name"]))
    return {
        "kind": ISSUE_KIND_GAPS,
        ...
        "limit": limit,
        "pageCount": len(ranked_pages),
        "pages": ranked_pages[:limit],
        "errorSignatureCount": len(ranked_signatures),
        "errorSignatures": ranked_signatures[:limit],
        "notCompared": list(document.get("notCompared") or []),
    }


def render_gap_issue_markdown(issue: Mapping[str, Any], run_url: str | None = None) -> str:
    summary = issue["summary"]
    lines = [
        issue_marker(ISSUE_KIND_GAPS),
        "",
        "## Privacy test pages — probes Chromium answers and Broiler does not",
        "",
        _issue_provenance(issue),
        "",
        f"- Gaps: **{summary.get('gaps', 0)}** probe(s) across **{issue['pageCount']}** page(s), "
        f"{summary.get('gapsFromErrors', 0)} of them because the probe threw in Broiler",
        f"- In parity: {summary.get('parity', 0)} probe(s) over "
        f"{summary.get('pagesCompared', 0)}/{summary.get('pages', 0)} compared page(s)",
        f"- Answered by neither engine: {summary.get('neither', 0)} · "
        f"answered only by Broiler: {summary.get('broilerOnly', 0)} · "
        f"answered in a different shape: {summary.get('divergentTypes', 0)}",
        "",
        f"> {COMPARISON_NOTICE}",
        "",
    ]

    if issue["errorSignatures"]:
        lines += [
            f"### Top {len(issue['errorSignatures'])} of {issue['errorSignatureCount']} "
            "thrown-error signature(s)",
            "",
            "_Gaps where the probe threw in Broiler, grouped by what it threw. One "
            "signature is usually one missing API gating every probe that touches it._",
            "",
        ]
        for index, entry in enumerate(issue["errorSignatures"], start=1):
            lines.append(
                f"{index}. `{_escape_table_cell(entry['signature'])}` — {entry['count']} gap(s) "
                f"on {', '.join(f'`{page}`' for page in entry['pages'])}"
            )
            if entry["examples"]:
                lines.append(f"   - Probes: {_probe_list(entry['examples'], 3)}")
        lines.append("")

    if issue["pages"]:
        lines += [
            f"### Top {len(issue['pages'])} of {issue['pageCount']} page(s) by gap count",
            "",
            "| Page | Gaps | Thrown | Probes |",
            "| --- | --- | --- | --- |",
        ]
        lines += [
            f"| [{page['name']}]({page['url']}) (`{page['id']}`) | {page['gaps']} "
            f"| {page['errors']} | {_probe_list(page['probes'])} |"
    ...
    if issue["notCompared"]:
        lines += [
            "### Pages with no Chromium reference",
            "",
            "_Not counted above: nothing was compared on them at all._",
            "",
            "| Page | Why |",
            "| --- | --- |",
        ]
        lines += [
            f"| [{entry['name']}]({entry['url']}) | {_single_line(entry['reason'], 160)} |"
            for entry in issue["notCompared"]
        ]
        lines.append("")

    return "\n".join(lines + _issue_footer(run_url)) + "\n"


def build_regression_issue(
    aggregate: Mapping[str, Any], *, limit: int = DEFAULT_REGRESSION_ISSUE_LIMIT
) -> dict[str, Any]:
    """Collect the probes and pages that went backwards against the baseline."""

    pages: list[dict[str, Any]] = []
    for result in aggregate["results"]:
        regressions = result.get("comparison", {}).get("regressions", [])
        if not regressions:
            continue
        probes = [change for change in regressions if change["test"] != "(page)"]
        pages.append({
            "id": result["id"],
            "name": result["name"],
            "url": result["url"],
            "status": result["status"],
            "reason": _single_line(result.get("reason") or "", 200) or None,
            "stoppedRunning": any(change["test"] == "(page)" for change in regressions),
            "count": len(regressions),
            "probeCount": len(probes),
            "probes": probes[:limit],
        })

    pages.sort(key=lambda page: (-page["count"], page["name"]))
    return {
        "kind": ISSUE_KIND_REGRESSIONS,
        "generatedAt": aggregate["environment"].get("timestamp"),
        "summary": dict(aggregate["summary"]),
        "reference": dict(aggregate.get("reference", {})),
        "environment": dict(aggregate["environment"]),
        "baseline": dict(aggregate.get("baseline", {})),
        "limit": limit,
        "count": sum(page["count"] for page in pages),
        "pages": pages,
    }


def render_regression_issue_markdown(issue: Mapping[str, Any], run_url: str | None = None) -> str:
    summary = issue["summary"]
    baseline = issue.get("baseline", {})
    lines = [
        issue_marker(ISSUE_KIND_REGRESSIONS),
        "",
        "## Privacy test pages — probes that stopped producing a value",
        "",
        _issue_provenance(issue),
        "",
        f"- Regressions: **{issue['count']}** across **{len(issue['pages'])}** page(s)",
        f"- Improvements in the same run: {summary.get('improvements', 0)}",
        f"- Upstream churn (never a regression): {summary.get('newTests', 0)} new, "
        f"{summary.get('removedTests', 0)} removed",
        f"- Baseline: `{baseline.get('path') or 'n/a'}` generated {baseline.get('generatedAt') or 'n/a'}",
        "",
        f"> {OBSERVATIONAL_NOTICE}",
        "",
    ]
    ...
    if not issue["pages"]:
        lines += ["No probe stopped producing a value in this run.", ""]
        return "\n".join(lines + _issue_footer(run_url)) + "\n"
    ...
    for page in issue["pages"]:
        lines += [
            ...
            "",
            f"- Page: <{page['url']}>",
            f"- Broiler status: `{page['status']}`"
            + (f" — {page['reason']}" if page["reason"] else ""),
        ]
        if page["stoppedRunning"]:
            # Every probe on the page regressed, for one reason, and listing all
            # of them would bury the pages whose probes regressed one by one —
            # which are the ones that name a specific thing that broke.
            lines += [
                f"- **The page stopped running altogether**, so all {page['probeCount']} probe(s) "
                "that used to produce a value went unmeasured. Fix the page, then read the probes.",
                "",
            ]
            continue
        lines.append("")
        if page["probes"]:
            lines += ["| Probe | Was | Now |", "| --- | --- | --- |"]
            lines += [
                f"| `{change['test']}` | {change['from']} | {change['to']} |"
                for change in page["probes"]
            ]
            if page["probeCount"] > len(page["probes"]):
                lines.append(
                    f"| … {page['probeCount'] - len(page['probes'])} more | | |"
                )
            lines.append("")
...
def build_unmeasured_issue(aggregate: Mapping[str, Any]) -> dict[str, Any]:
    """Collect the pages this run did not measure, and why.

    A page Broiler could not run, and a page with no Chromium reference, both
    report zero gaps — the same number a page in full parity reports. Naming
    them is what keeps that zero from reading as a result.  A run that disabled
    the comparison on purpose (``--no-reference``) is not such a case, so its
    uncompared pages are left out rather than reported as a finding.
    """

    disabled = bool(aggregate.get("reference", {}).get("disabled"))
    pages: list[dict[str, Any]] = []
    for result in aggregate["results"]:
        parity = result.get("parity", {})
        compared = bool(parity.get("available"))
        broiler_failed = result["status"] != "ok"
        if not broiler_failed and (compared or disabled):
            continue
        pages.append({
            "id": result["id"],
            "name": result["name"],
            "url": result["url"],
            "broilerStatus": result["status"],
            "broilerReason": _single_line(result.get("reason") or "", 200) or None,
            "comparedWithChromium": compared,
            "referenceReason": None
            if compared
            else _single_line(parity.get("reason") or "no Chromium reference was captured", 200),
        })

    return {
        "kind": ISSUE_KIND_UNMEASURED,
        "generatedAt": aggregate["environment"].get("timestamp"),
        "summary": dict(aggregate["summary"]),
        "reference": dict(aggregate.get("reference", {})),
        "environment": dict(aggregate["environment"]),
        "comparisonDisabled": disabled,
        "referenceRun": (aggregate.get("reference", {}) or {}).get("run"),
        "count": len(pages),
        "pages": pages,
    }


def render_unmeasured_issue_markdown(issue: Mapping[str, Any], run_url: str | None = None) -> str:
    summary = issue["summary"]
    reference_run = issue.get("referenceRun") or {}
    lines = [
        issue_marker(ISSUE_KIND_UNMEASURED),
        "",
        "## Privacy test pages — pages this run could not measure",
        "",
        _issue_provenance(issue),
        "",
        f"- Pages not measured: **{issue['count']}** of {summary.get('pages', 0)}",
        f"- Pages Broiler ran: {summary.get('pagesOk', 0)}/{summary.get('pages', 0)} · "
        f"compared with Chromium: {summary.get('pagesCompared', 0)}/{summary.get('pages', 0)}",
        "",
        "_A page reported here has no gap count to read: its probes were not carried "
        "out, or there was nothing to compare them against._",
        "",
    ]

    if reference_run and (reference_run.get("exitCode") or reference_run.get("timedOut")):
        lines += [
            f"The Chromium capture exited with `{reference_run.get('exitCode')}`"
            + (" after timing out" if reference_run.get("timedOut") else "")
            + " — see `reference.log` in the run artifact.",
            "",
        ]

    if not issue["pages"]:
        lines += ["Every page ran and was compared with Chromium.", ""]
        return "\n".join(lines + _issue_footer(run_url)) + "\n"

    lines += ["| Page | Broiler | Chromium |", "| --- | --- | --- |"]
    for page in issue["pages"]:
        broiler = (
            f"`{page['broilerStatus']}`"
            if page["broilerStatus"] == "ok"
            else f"`{page['broilerStatus']}` — {_escape_table_cell(page['broilerReason'] or 'no reason recorded')}"
        )
        chromium = (
            "compared"
            if page["comparedWithChromium"]
            else f"not compared — {_escape_table_cell(page['referenceReason'])}"
        )
        lines.append(f"| [{page['name']}]({page['url']}) (`{page['id']}`) | {broiler} | {chromium} |")
    lines.append("")

    return "\n".join(lines + _issue_footer(run_url)) + "\n"


def build_issue_documents(
    aggregate: Mapping[str, Any],
    gap_document: Mapping[str, Any],
    *,
    gap_limit: int = DEFAULT_GAP_ISSUE_LIMIT,
    regression_limit: int = DEFAULT_REGRESSION_ISSUE_LIMIT,
    run_url: str | None = None,
) -> list[dict[str, Any]]:
    """Build every issue this run would file, in the order they matter.

    Each entry carries its own ``file``, ``title``, ``marker`` and whether it is
    worth filing at all, so the workflow needs no second, drifting copy of the
    rules that decide when a finding is a finding.
    """

    day = (aggregate["environment"].get("timestamp") or utc_now())[:10]
    summary = aggregate["summary"]

    regressions = build_regression_issue(aggregate, limit=regression_limit)
    unmeasured = build_unmeasured_issue(aggregate)
    gaps = build_gap_issue(gap_document, limit=gap_limit)

    return [
        {
            "kind": ISSUE_KIND_REGRESSIONS,
            "marker": issue_marker(ISSUE_KIND_REGRESSIONS),
            "file": f"{ISSUE_KIND_REGRESSIONS}.md",
            "title": f"Privacy test pages: {regressions['count']} regression(s) on "
            f"{len(regressions['pages'])} page(s) ({day})",
            "create": regressions["count"] > 0,
            "counts": {"regressions": regressions["count"], "pages": len(regressions["pages"])},
            "body": render_regression_issue_markdown(regressions, run_url),
        },
        {
            "kind": ISSUE_KIND_UNMEASURED,
            "marker": issue_marker(ISSUE_KIND_UNMEASURED),
            "file": f"{ISSUE_KIND_UNMEASURED}.md",
            "title": f"Privacy test pages: {unmeasured['count']} page(s) this run could not "
            f"measure ({day})",
            "create": unmeasured["count"] > 0,
            "counts": {"pages": unmeasured["count"]},
            "body": render_unmeasured_issue_markdown(unmeasured, run_url),
        },
        {
            "kind": ISSUE_KIND_GAPS,
            "marker": issue_marker(ISSUE_KIND_GAPS),
            "file": f"{ISSUE_KIND_GAPS}.md",
            "title": f"Privacy test pages: {summary['gaps']} probe(s) behind Chromium on "
            f"{gaps['pageCount']} page(s) ({day})",
            # Only a run that actually compared something can claim a gap: with
            # no reference at all the gap count is 0 for the same reason it would
            # be on a perfect run, and the unmeasured issue is the honest report.
            "create": summary["gaps"] > 0 and summary["pagesCompared"] > 0,
            "counts": {"gaps": summary["gaps"], "pages": gaps["pageCount"]},
            "body": render_gap_issue_markdown(gaps, run_url),
        },
    ]


def write_issue_documents(
    output_root: Path, documents: Sequence[Mapping[str, Any]], *, run_url: str | None = None
) -> dict[str, Any]:
    """Write the issue bodies and the index the workflow files them from.

    Only a body that is worth filing is written: a "0 regressions" file left
    beside a run that had none is the kind of artifact that later gets pasted
    into an issue as if it were one.
    """

    issue_dir = output_root / ISSUE_DIR_NAME
    ...
    for document in documents:
        entry = {
            ...
            "create": bool(document["create"]),
            "counts": dict(document["counts"]),
            "body": f"{ISSUE_DIR_NAME}/{document['file']}" if document["create"] else None,
        }
        if document["create"]:
            (issue_dir / document["file"]).write_text(document["body"], encoding="utf-8")
    ...
    write_json(issue_dir / ISSUE_INDEX_NAME, index)
    return index


# Step outputs, named for the finding rather than for the file that carries it,
# so a workflow step reads as `if: ... create_gaps_issue == 'true'`.
ISSUE_OUTPUT_NAMES = {
    ISSUE_KIND_REGRESSIONS: {"regressions": "regression_count", "pages": "regressed_page_count"},
    ISSUE_KIND_UNMEASURED: {"pages": "unmeasured_page_count"},
    ISSUE_KIND_GAPS: {"gaps": "gap_count", "pages": "gapped_page_count"},
}


def write_github_outputs(path: Path, index: Mapping[str, Any]) -> None:
    """Append the counts and create_* flags a workflow files the issues from."""

    lines = [f"issue_count={sum(1 for entry in index['issues'] if entry['create'])}"]
    for entry in index["issues"]:
        lines.append(f"create_{entry['kind']}_issue={'true' if entry['create'] else 'false'}")
        names = ISSUE_OUTPUT_NAMES.get(entry["kind"], {})
        lines += [
            f"{names[key]}={value}" for key, value in entry["counts"].items() if key in names
        ]
...
def clear_previous_run(output_root: Path, *, keep_references: bool = False) -> None:
    ...
    # A stale issue body is worse than a stale report: this run's workflow would
    # file last run's finding as if this run had made it.
    issue_root = output_root / ISSUE_DIR_NAME
...
def create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--gap-issue-limit",
        type=int,
        default=DEFAULT_GAP_ISSUE_LIMIT,
        help="maximum error signatures and pages to rank in the gaps issue "
        f"(default: {DEFAULT_GAP_ISSUE_LIMIT})",
    )
    parser.add_argument(
        "--regression-issue-limit",
        type=int,
        default=DEFAULT_REGRESSION_ISSUE_LIMIT,
        help="maximum regressed probes to list per page in the regressions issue "
        f"(default: {DEFAULT_REGRESSION_ISSUE_LIMIT})",
    )
    parser.add_argument(
        "--run-url",
        default=os.environ.get("PRIVACY_RUN_URL"),
        help="workflow run URL for the issue footers (default: $PRIVACY_RUN_URL)",
    )
    parser.add_argument(
        "--github-output",
        default=None,
        help="path to $GITHUB_OUTPUT; receives the counts and the create_* flags "
        "the workflow files the issues from",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.gap_issue_limit < 1:
        parser.error("--gap-issue-limit must be a positive integer")
    if args.regression_issue_limit < 1:
        parser.error("--regression-issue-limit must be a positive integer")
    ...
    issues = build_issue_documents(
        aggregate,
        gaps,
        gap_limit=args.gap_issue_limit,
        regression_limit=args.regression_issue_limit,
        run_url=args.run_url,
    )
    if args.update_baseline:
        # A baseline refresh runs the corpus in order to redefine what "as
        # before" means, so its comparison describes the baseline being replaced
        # rather than a finding: report it, never file it.
        for document in issues:
            document["create"] = False
    issue_index = write_issue_documents(output_root, issues, run_url=args.run_url)
    ...
    filed = [entry for entry in issue_index["issues"] if entry["create"]]
    print(
        "Issues to file: "
        + (", ".join(f"{entry['kind']} ({entry['title']})" for entry in filed) if filed else "none")
    )

    # Written before the failure checks below: a run that fails on a regression
    # is exactly the run whose issue the workflow still has to file.
    if args.github_output:
        write_github_outputs(Path(args.github_output), issue_index)
