...
internal static class CodeShellFactory
{
    ...
    /// save.
    /// </summary>
    public static ValueTask<CodeWorkspace> OpenWorkspaceAsync(CodeShell shell, string? path)
    {
        string root = path is { Length: > 0 } && Directory.Exists(path)
            ? Path.GetFullPath(path)
            : ScratchRoot();

        return WorkspaceBootstrap.OpenAsync(shell, new FileSystemWorkspaceStorage(root));
    }
