...
public sealed partial class DomBridge
{
    ...
    private JSObject BuildStyleSheetObject(DomElement styleElement)
    {
        ...
        liveCssRules.FastAddProperty((KeyString)"length",
            new JSFunction((in _) => JsStyleSheetsGetLength002Core(CurrentRules, in _), "get length"),
            null, JSPropertyAttributes.EnumerableConfigurableProperty);

        liveCssRules.FastAddValue((KeyString)"item",
            new JSFunction((in a) => JsStyleSheetsItem003Core(SyncLiveCssRulesIndices, liveCssRules, CurrentRules, in a), "item", 1),
        ...
        sheet.FastAddProperty((KeyString)"cssRules",
            new JSFunction((in _) => JsStyleSheetsGetCssRules004Core(SyncLiveCssRulesIndices, liveCssRules, in _), "get cssRules"),
            null, JSPropertyAttributes.EnumerableConfigurableProperty);
        ...
        sheet.FastAddValue((KeyString)"insertRule",
            new JSFunction((in a) => JsStyleSheetsInsertRule005Core(CurrentRules, MarkRulesMutated, SyncLiveCssRulesIndices, in a), "insertRule", 2),
            JSPropertyAttributes.EnumerableConfigurableValue);

        // deleteRule(index) — removes a rule from the shared model
        sheet.FastAddValue((KeyString)"deleteRule",
            new JSFunction((in a) => JsStyleSheetsDeleteRule006Core(CurrentRules, MarkRulesMutated, SyncLiveCssRulesIndices, in a), "deleteRule", 1),
            JSPropertyAttributes.EnumerableConfigurableValue);
    ...
    /// </summary>
    private static JSObject BuildCssRuleObject(CssRule rule, JSObject parentStyleSheet, JSObject? parentRule = null)
    {
        ...
        switch (kind)
        {
            ...
            case CssomRuleType.Property:
                {
                    var descriptors = ParseStyle(CssSerializer.Serialize(((CssAtRule)rule).Declarations ?? new CssDeclarationBlock([])));
            ...
            case CssomRuleType.CounterStyle:
                {
                    ...
                    var descriptors = ParseStyle(CssSerializer.Serialize(atRule.Declarations ?? new CssDeclarationBlock([])));
